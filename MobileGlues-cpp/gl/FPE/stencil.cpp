...
void APIENTRY_FPE4ES fpe4es_glStencilMask(GLuint mask) {
...
void APIENTRY_FPE4ES fpe4es_glStencilFunc(GLenum func, GLint ref, GLuint mask) {
    if(!glstate->list.pending) 
...
void APIENTRY_FPE4ES fpe4es_glStencilFuncSeparate(GLenum face, GLenum func, GLint ref, GLuint mask) {
    if(face!=GL_FRONT && face!=GL_BACK && face!=GL_FRONT_AND_BACK) {
        errorShim(GL_INVALID_ENUM);
        return;
    }
    ...
    if(gles_glStencilFuncSeparate) {
        ...
        // fake function..., call it only for front or front_and_back, just ignore back (crappy, I know)
        if (face==GL_FRONT)
            fpe4es_glStencilFunc(func, ref, mask);
        else
            noerrorShim();
    }
}
...
void APIENTRY_FPE4ES fpe4es_glStencilOp(GLenum fail, GLenum zfail, GLenum zpass) {
    if(!glstate->list.pending) 
...
void APIENTRY_FPE4ES fpe4es_glStencilOpSeparate(GLenum face, GLenum sfail, GLenum zfail, GLenum zpass) {
    if(face!=GL_FRONT && face!=GL_BACK && face!=GL_FRONT_AND_BACK) {
        errorShim(GL_INVALID_ENUM);
        return;
    }
    ...
    if(gles_glStencilOpSeparate) {
        ...
        //fake, again
        if (face==GL_FRONT)
            fpe4es_glStencilOp(sfail, zfail, zpass);
        else
            noerrorShim();
    }
}
...
void APIENTRY_FPE4ES fpe4es_glClearStencil(GLint s) {
    if(!glstate->list.pending) 
