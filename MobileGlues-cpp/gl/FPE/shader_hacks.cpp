...
static const hack_t fpe4es_hacks[] = {
...
// For Stellaris
static const char* fpe4es_sign_1[] = {
    "if (Data.Type == 1)",
    "if (Data.BlendMode == 0)",
};
static const char* fpe4es_hacks_1[] = {
    "if (Data.Type == 1)",
    "if (Data.Type == 1.0)",
...
// For Psychonauts
static const char* fpe4es_sign_2[] = {
...
static const char* fpe4es_sign_2_main = "void main()\n"
                                        "{\n";

static const char* fpe4es_hacks_2_1[] = {
    "vec4 ps_t3;",
    "vec4 ps_t2;",
    "vec4 ps_t1;",
    "vec4 ps_t0;",
...
static const char* fpe4es_hacks_2_2[] = {
    "\tps_t3 = gl_TexCoord[3];",
    "\tps_t2 = gl_TexCoord[2];",
    "\tps_t1 = gl_TexCoord[1];",
    "\tps_t0 = gl_TexCoord[0];",
...
static char* ShaderHacks_1(char* shader, char* Tmp, int* tmpsize) {
    ...
    for (int i = 0; i < sizeof(fpe4es_sign_1) / sizeof(fpe4es_sign_1[0]); i++)
        if (!strstr(Tmp, fpe4es_sign_1[i])) return Tmp;
    // Do the replace
    for (int i = 0; i < sizeof(fpe4es_hacks_1) / sizeof(fpe4es_hacks_1[0]); i += 2)
        if (strstr(Tmp, fpe4es_hacks_1[i])) {
            if (Tmp == shader) {
                Tmp = (char*)malloc(*tmpsize);
                strcpy(Tmp, shader);
            } // hacking!
            Tmp = fpe4es_inplace_replace_simple(Tmp, tmpsize, fpe4es_hacks_1[i], fpe4es_hacks_1[i + 1]);
        }
    return Tmp;
}

static char* ShaderHacks_2_1(char* shader, char* Tmp, int* tmpsize, int i) {
    char* p = strstr(Tmp, fpe4es_sign_2[i]);
    if (!p) return Tmp; // not found
    char* m = strstr(Tmp, fpe4es_sign_2_main);
    ...
    if (Tmp == shader) {
        Tmp = (char*)malloc(*tmpsize);
        strcpy(Tmp, shader);
        m = strstr(Tmp, fpe4es_sign_2_main);
    } // hacking!
    m += strlen(fpe4es_sign_2_main);
    Tmp = fpe4es_inplace_insert(m, fpe4es_hacks_2_2[i], Tmp, tmpsize);
    Tmp = fpe4es_inplace_replace_simple(Tmp, tmpsize, fpe4es_sign_2[i], fpe4es_hacks_2_1[i]);
    return Tmp;
}

static char* ShaderHacks_2(char* shader, char* Tmp, int* tmpsize) {
    // check for each signature
    for (int i = 0; i < sizeof(fpe4es_sign_2) / sizeof(fpe4es_sign_2[0]); i++)
...
char* ShaderHacks(char* shader) {
    ...
    // generic
    for (int i = 0; i < sizeof(fpe4es_hacks) / sizeof(fpe4es_hacks[0]); ++i) {
        char* f = fpe4es_hacks[i].sign;
        int n = fpe4es_hacks[i].n;
        if (strstr(Tmp, f)) {
            if (Tmp == shader) {
                Tmp = (char*)malloc(tmpsize);
                strcpy(Tmp, shader);
            ...
            for (int j = 0; j < n; j += 2) {
                if (j) f = fpe4es_hacks[i].next[j - 1];
                Tmp = fpe4es_inplace_replace_simple(Tmp, &tmpsize, f, fpe4es_hacks[i].next[j]);
            }
        }
    }
    return Tmp;
