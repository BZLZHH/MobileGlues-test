...
void APIENTRY_FPE4ES fpe4es_glCullFace(GLenum mode) {
    if (!glstate->list.pending) PUSH_IF_COMPILING(glCullFace);
...
void APIENTRY_FPE4ES fpe4es_glFrontFace(GLenum mode) {
    if (!glstate->list.pending) PUSH_IF_COMPILING(glFrontFace);
