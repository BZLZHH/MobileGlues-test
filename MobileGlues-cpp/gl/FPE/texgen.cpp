...
void APIENTRY_FPE4ES fpe4es_glTexGenfv(GLenum coord, GLenum pname, const GLfloat* param) {
    ...
    */
    ...
    // printf("glTexGenf(%s, %s, %s/%f), texture=%i\n", PrintEnum(coord), PrintEnum(pname), PrintEnum(param[0]),
    // param[0], glstate->texture.active);
    ERROR_IN_BEGIN
    ...
    case GL_TEXTURE_GEN_MODE: {
        ...
        if (glstate->fpe_state) {
            ...
            case GL_OBJECT_LINEAR:
                mode = FPE_TG_OBJLINEAR;
                break;
            case GL_EYE_LINEAR:
                mode = FPE_TG_EYELINEAR;
                break;
            case GL_SPHERE_MAP:
                mode = FPE_TG_SPHEREMAP;
                break;
            case GL_NORMAL_MAP:
                mode = FPE_TG_NORMALMAP;
                break;
            case GL_REFLECTION_MAP:
                mode = FPE_TG_REFLECMAP;
                break;
            }
        }
        switch (coord) {
        case GL_S:
            glstate->texgen[n].S = param[0];
            if (mode != -1) {
                glstate->fpe_state->texgen[n].texgen_s_mode = mode;
            }
            break;
        case GL_T:
            glstate->texgen[n].T = param[0];
            if (mode != -1) {
                glstate->fpe_state->texgen[n].texgen_t_mode = mode;
            }
            break;
        case GL_R:
            ...
            glstate->texgen[n].R = param[0];
            if (mode != -1) {
                glstate->fpe_state->texgen[n].texgen_r_mode = mode;
            }
            break;
        case GL_Q:
            ...
            glstate->texgen[n].Q = param[0];
            if (mode != -1) {
                glstate->fpe_state->texgen[n].texgen_q_mode = mode;
            }
            break;
        default:
            errorShim(GL_INVALID_ENUM);
...
void APIENTRY_FPE4ES fpe4es_glGetTexGenfv(GLenum coord, GLenum pname, GLfloat* params) {
    ...
    case GL_TEXTURE_GEN_MODE:
        switch (coord) {
        case GL_S:
            *params = glstate->texgen[glstate->texture.active].S;
            break;
        case GL_T:
            *params = glstate->texgen[glstate->texture.active].T;
            break;
        case GL_R:
            *params = glstate->texgen[glstate->texture.active].R;
            break;
        case GL_Q:
            *params = glstate->texgen[glstate->texture.active].Q;
            break;
        default:
            *params = GL_EYE_LINEAR;
        }
        break;
    case GL_OBJECT_PLANE:
        switch (coord) {
...
void eye_loop_dual(const GLfloat* verts, const GLfloat* param1, const GLfloat* param2, GLfloat* out, GLint count,
                   GLushort* indices) {
    // based on https://www.opengl.org/wiki/Mathematics_of_glTexGen
    // First get the ModelviewMatrix
...
static inline void tex_coord_loop(GLfloat* verts, GLfloat* norm, GLfloat* out, GLint count, GLenum type,
                                  GLfloat* param_o, GLfloat* param_e, GLushort* indices) {
    switch (type) {
    case GL_OBJECT_LINEAR:
...
void gen_tex_coords(GLfloat* verts, GLfloat* norm, GLfloat** coords, GLint count, GLint* needclean, int texture,
    ...
    // printf("gen_tex_coords(%p, %p, %p, %d, %p, %d, %p, %d) texgen = S:%s T:%s R:%s Q:%s, enabled:%c%c%c%c,
    // tex=%02X\n", verts, norm, *coords, count, needclean, texture, indices, ilen,
    // (glstate->enable.texgen_s[texture])?PrintEnum(glstate->texgen[texture].S):"-",
    // (glstate->enable.texgen_t[texture])?PrintEnum(glstate->texgen[texture].T):"-",
    // (glstate->enable.texgen_r[texture])?PrintEnum(glstate->texgen[texture].R):"-",
    // (glstate->enable.texgen_q[texture])?PrintEnum(glstate->texgen[texture].Q):"-",
    // (glstate->enable.texgen_s[texture])?'S':'-', (glstate->enable.texgen_t[texture])?'T':'-',
    // (glstate->enable.texgen_r[texture])?'R':'-', (glstate->enable.texgen_q[texture])?'Q':'-',
    // glstate->enable.texture[texture]);
    ...
    if (!glstate->enable.texgen_s[texture] && !glstate->enable.texgen_t[texture] &&
        !glstate->enable.texgen_r[texture] && !glstate->enable.texgen_q[texture]) {
        if ((*coords) == NULL) *coords = (GLfloat*)malloc(count * 4 * sizeof(GLfloat));
        if (indices)
    ...
    if ((glstate->enable.texgen_s[texture] && (glstate->texgen[texture].S == GL_SPHERE_MAP)) &&
        (glstate->enable.texgen_t[texture] && (glstate->texgen[texture].T == GL_SPHERE_MAP))) {
        if (!IS_TEX2D(glstate->enable.texture[texture])) return;
        if ((*coords) == NULL) *coords = (GLfloat*)malloc(count * 4 * sizeof(GLfloat));
    ...
    if ((glstate->enable.texgen_s[texture] && (glstate->texgen[texture].S == GL_REFLECTION_MAP)) &&
        ...
        if (hardext.cubemap) {
            *needclean = 1;
            ...
            if (old_tex != texture) fpe4es_glActiveTexture(GL_TEXTURE0 + texture);
            realize_active();
            ...
            if (!(globals4es.texmat || glstate->texture_matrix[texture]->identity)) {
                ...
                if (old_mat != GL_TEXTURE) fpe4es_glMatrixMode(GL_TEXTURE);
                gles_glLoadMatrixf(getTexMat(texture));
                if (old_mat != GL_TEXTURE) fpe4es_glMatrixMode(old_mat);
            }

            if (old_tex != texture) fpe4es_glActiveTexture(GL_TEXTURE0 + old_tex);
        } else {
            if (!IS_TEX2D(glstate->enable.texture[texture])) return;
            if ((*coords) == NULL) *coords = (GLfloat*)malloc(count * 4 * sizeof(GLfloat));
    ...
    if (!IS_ANYTEX(glstate->enable.texture[texture])) return;
    if ((*coords) == NULL) *coords = (GLfloat*)malloc(count * 4 * sizeof(GLfloat));
    if ((glstate->enable.texgen_s[texture] && glstate->texgen[texture].S == GL_EYE_LINEAR) &&
        ...
        eye_loop_dual(verts, glstate->texgen[texture].S_E, glstate->texgen[texture].T_E, (*coords),
                      (indices) ? ilen : count, indices);
    } else {
        if (glstate->enable.texgen_s[texture])
            tex_coord_loop(verts, norm, (*coords), (indices) ? ilen : count, glstate->texgen[texture].S,
                           glstate->texgen[texture].S_O, glstate->texgen[texture].S_E, indices);
        if (glstate->enable.texgen_t[texture])
            tex_coord_loop(verts, norm, (*coords) + 1, (indices) ? ilen : count, glstate->texgen[texture].T,
                           glstate->texgen[texture].T_O, glstate->texgen[texture].T_E, indices);
    }
    if (glstate->enable.texgen_r[texture])
        tex_coord_loop(verts, norm, (*coords) + 2, (indices) ? ilen : count, glstate->texgen[texture].R,
                       glstate->texgen[texture].R_O, glstate->texgen[texture].R_E, indices);
    else
    ...
    if (glstate->enable.texgen_q[texture])
        tex_coord_loop(verts, norm, (*coords) + 3, (indices) ? ilen : count, glstate->texgen[texture].Q,
                       glstate->texgen[texture].Q_O, glstate->texgen[texture].Q_E, indices);
    else
        for (int i = 0; i < ((indices) ? ilen : count); i++) {
            GLushort k = indices ? indices[i] : i;
            (*coords)[k * 4 + 3] = 1.0f;
...
void APIENTRY_FPE4ES fpe4es_glLoadTransposeMatrixd(const GLdouble* m) {
    ...
    for (int i = 0; i < 16; i++)
        mf[i] = m[i];
    fpe4es_glLoadTransposeMatrixf(mf);
}
