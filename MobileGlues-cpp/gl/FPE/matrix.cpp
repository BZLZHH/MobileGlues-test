...
static int update_current_identity(int I) {
    ...
    case GL_TEXTURE:
        return glstate->texture_matrix[glstate->texture.active]->identity =
                   (I) ? 1 : is_identity(TOP(texture_matrix[glstate->texture.active]));
    default:
        if (glstate->matrix_mode >= GL_MATRIX0_ARB && glstate->matrix_mode < GL_MATRIX0_ARB + MAX_ARB_MATRIX)
            return glstate->arb_matrix[glstate->matrix_mode - GL_MATRIX0_ARB]->identity =
                       (I) ? 1 : is_identity(TOP(arb_matrix[glstate->matrix_mode - GL_MATRIX0_ARB]));
        return 0;
    }
}
...
void APIENTRY_FPE4ES fpe4es_glMatrixMode(GLenum mode) {
...
void APIENTRY_FPE4ES fpe4es_glPushMatrix(void) {
...
void APIENTRY_FPE4ES fpe4es_glPopMatrix(void) {
...
void APIENTRY_FPE4ES fpe4es_glLoadMatrixf(const GLfloat* m) {
    ...
    if (glstate->matrix_mode == GL_MODELVIEW) glstate->normal_matrix_dirty = glstate->inv_mv_matrix_dirty = 1;
    ...
    if (send_to_hardware()) {
        ...
        if (id)
            gles_glLoadIdentity(); // in case the driver as some special optimisations
        else
            gles_glLoadMatrixf(m);
    }
}

void APIENTRY_FPE4ES fpe4es_glMultMatrixf(const GLfloat* m) {
...
void APIENTRY_FPE4ES fpe4es_glLoadIdentity(void) {
...
void APIENTRY_FPE4ES fpe4es_glTranslatef(GLfloat x, GLfloat y, GLfloat z) {
    ...
    tmp[12 + 1] = y;
    tmp[12 + 2] = z;
    fpe4es_glMultMatrixf(tmp);
}

void APIENTRY_FPE4ES fpe4es_glScalef(GLfloat x, GLfloat y, GLfloat z) {
...
void APIENTRY_FPE4ES fpe4es_glRotatef(GLfloat angle, GLfloat x, GLfloat y, GLfloat z) {
    DBG(printf("glRotatef(%f, %f, %f, %f), list=%p\n", angle, x, y, z, glstate->list.active);)
    // create a rotation matrix than multiply it...
    GLfloat tmp[16];
    memset(tmp, 0, 16 * sizeof(GLfloat));
    if ((x == 0 && y == 0 && z == 0) || angle == 0) return; // nothing to do
    // normalize x y z
...
void APIENTRY_FPE4ES fpe4es_glOrthof(GLfloat left, GLfloat right, GLfloat bottom, GLfloat top, GLfloat nearVal,
                                   GLfloat farVal) {
    ...
    tmp[0 + 12] = -(right + left) / (right - left);
    tmp[1 + 4] = 2.0f / (top - bottom);
    tmp[1 + 12] = -(top + bottom) / (top - bottom);
    tmp[2 + 8] = -2.0f / (farVal - nearVal);
    tmp[2 + 12] = -(farVal + nearVal) / (farVal - nearVal);
    tmp[3 + 12] = 1.0f;

    fpe4es_glMultMatrixf(tmp);
}

void APIENTRY_FPE4ES fpe4es_glFrustumf(GLfloat left, GLfloat right, GLfloat bottom, GLfloat top, GLfloat nearVal,
                                     GLfloat farVal) {
    ...
    tmp[0 + 8] = (right + left) / (right - left);
    tmp[1 + 4] = 2.0f * nearVal / (top - bottom);
    tmp[1 + 8] = (top + bottom) / (top - bottom);
    tmp[2 + 8] = -(farVal + nearVal) / (farVal - nearVal);
    tmp[2 + 12] = -2.0f * farVal * nearVal / (farVal - nearVal);
    tmp[3 + 8] = -1.0f;
...
AliasExport(void, glOrthof, ,
            (GLfloat left, GLfloat right, GLfloat bottom, GLfloat top, GLfloat nearVal, GLfloat farVal));
AliasExport(void, glFrustumf, ,
            (GLfloat left, GLfloat right, GLfloat bottom, GLfloat top, GLfloat nearVal, GLfloat farVal));
