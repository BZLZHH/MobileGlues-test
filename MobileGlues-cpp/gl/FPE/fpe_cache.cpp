...
static const char PSA_SIGN[] = "FPE4ES PrecompiledShaderArchive";
...
void fpe_disposeCache(fpe_cache_t* cache, int freeprog) {
    ...
    kh_foreach_value(
        cache, m, if (freeprog) {
            if (m->glprogram) fpe4es_glDeleteProgram(m->glprogram->id);
...
typedef struct fpe4es_psa_s {
    int size;
    int dirty;
    kh_psalist_t* cache;
} fpe4es_psa_t;

static fpe4es_psa_t* psa = NULL;
...
void fpe_InitPSA(const char* name) {
    ...
    psa = (fpe4es_psa_t*)calloc(1, sizeof(fpe4es_psa_t));
...
int fpe_GetProgramPSA(GLuint program, fpe_state_t* state) {
    ...
    // try to load...
    return fpe4es_useProgramBinary(program, p->size, p->format, p->prog);
}
...
void fpe_AddProgramPSA(GLuint program, fpe_state_t* state) {
    ...
    int l = fpe4es_getProgramBinary(program, &p->size, &p->format, &p->prog);
