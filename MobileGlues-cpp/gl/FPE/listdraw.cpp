...
int list2VBO(renderlist_t* list) {
    ...
    for (int i = 0; i < imax; ++i) {
        ...
        if (base == basebase) vbo_base += r->real_size;
    }
    if (!vbo_base) // no data?!
        return 1;
...
void listActiveVBO(renderlist_t* list, save_vbo_t* saved) {
    ...
    for (int a = 0; a < list->maxtex; ++a) {
        if (list->tex[a]) {
            ...
            saved[ATT_MULTITEXCOORD0 + a].real_pointer =
                glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + a].real_pointer;
...
void draw_renderlist(renderlist_t* list) {
    ...
    // list, list->len, PrintEnum(list->mode), PrintEnum(list->mode_init), list->ilen, list->next, list->color,
    // list->secondary, list->fogcoord);
    ...
    fpe4es_glPushClientAttrib(GL_CLIENT_VERTEX_ARRAY_BIT);
    ...
    do {
        ...
        // push/pop attributes
        if (list->pushattribute) fpe4es_glPushAttrib(list->pushattribute);
        if (list->popattribute) fpe4es_glPopAttrib();
        ...
        if (list->render_op) {
            ...
            case 2:
                ...
                break;
            case 3:
                fpe4es_glPushName(list->render_arg);
                break;
            case 4:
                fpe4es_glLoadName(list->render_arg);
                break;
            }
        }
        if (list->fog_op) {
            fpe4es_glFogfv(GL_FOG_COLOR, list->fog_val);
        }
        if (list->pointparam_op) {
            switch (list->pointparam_op) {
            case 1: // GL_POINT_DISTANCE_ATTENUATION
                fpe4es_glPointParameterfv(GL_POINT_DISTANCE_ATTENUATION, list->pointparam_val);
                break;
            }
        }
        if (list->matrix_op) {
            ...
            case 1: // load
                fpe4es_glLoadMatrixf(list->matrix_val);
                break;
            case 2: // mult
                fpe4es_glMultMatrixf(list->matrix_val);
                break;
            }
        }
        if (list->set_tmu) {
            fpe4es_glActiveTexture(GL_TEXTURE0 + list->tmu);
        }
        if (list->set_texture) {
            fpe4es_glBindTexture(list->target_texture, list->texture);
        }
        // raster
        if (list->raster_op) {
            if (list->raster_op == 1) {
                fpe4es_glRasterPos3f(list->raster_xyz[0], list->raster_xyz[1], list->raster_xyz[2]);
            } else if (list->raster_op == 2) {
                fpe4es_glWindowPos3f(list->raster_xyz[0], list->raster_xyz[1], list->raster_xyz[2]);
            } else if (list->raster_op == 3) {
                fpe4es_glPixelZoom(list->raster_xyz[0], list->raster_xyz[1]);
            } else if ((list->raster_op & 0x10000) == 0x10000) {
                fpe4es_glPixelTransferf(list->raster_op & 0xFFFF, list->raster_xyz[0]);
            }
        }
        if (list->raster) {
        ...
        if (list->bitmaps) {
            for (int i = 0; i < list->bitmaps->count; i++) {
                ...
                fpe4es_glBitmap(l->width, l->height, l->xorig, l->yorig, l->xmove, l->ymove, l->bitmap);
            }
        }
        ...
        if (list->material) {
            ...
            rendermaterial_t* m;
            kh_foreach_value(
                map, m, switch (m->pname) {
                    case GL_SHININESS:
                        fpe4es_glMaterialf(m->face, m->pname, m->color[0]);
                        break;
                    default:
                        fpe4es_glMaterialfv(m->face, m->pname, m->color);
                })
        }
        if (list->colormat_face) fpe4es_glColorMaterial(list->colormat_face, list->colormat_mode);
        if (list->light) {
            khash_t(light)* lig = list->light;
            renderlight_t* m;
            kh_foreach_value(
                lig, m, switch (m->pname) {
                    default:
                        fpe4es_glLightfv(m->which, m->pname, m->color);
                })
        }
        if (list->lightmodel) {
            fpe4es_glLightModelfv(list->lightmodelparam, list->lightmodel);
        }

        if (list->linestipple_op) {
            fpe4es_glLineStipple(list->linestipple_factor, list->linestipple_pattern);
        }

        if (list->texenv) {
            khash_t(texenv)* tgn = list->texenv;
            rendertexenv_t* m;
            kh_foreach_value(tgn, m, fpe4es_glTexEnvfv(m->target, m->pname, m->params);)
        }

        if (list->texgen) {
            khash_t(texgen)* tgn = list->texgen;
            rendertexgen_t* m;
            kh_foreach_value(tgn, m, fpe4es_glTexGenfv(m->coord, m->pname, m->color);)
        }

        if (list->polygon_mode) {
            fpe4es_glPolygonMode(GL_FRONT_AND_BACK, list->polygon_mode);
        }
        ...
        if (list->color) {
            ...
            if (glstate->enable.color_sum && (list->secondary) && hardext.esversion == 1 && !list->use_glstate) {
                ...
                // list->color[0],list->color[1],list->color[2],list->color[3],
                // list->color[4],list->color[5],list->color[6],list->color[7]);
...
#define TEXTURE(A)                                                                                                     \
    if (cur_tex != A) {                                                                                                \
        fpe4es_glClientActiveTexture(A + GL_TEXTURE0);                                                                 \
        cur_tex = A;                                                                                                   \
    }
        stipple = false;
        ...
        if (stipple) {
            ...
            if (glstate->gleshard->active != stipple_tmu) {
                ...
                fpe4es_glActiveTexture(GL_TEXTURE0 + stipple_tmu);
            }
            TEXTURE(stipple_tmu);
            GLenum matmode;
            fpe4es_glGetIntegerv(GL_MATRIX_MODE, (GLint*)&matmode);
            fpe4es_glMatrixMode(GL_TEXTURE);
            fpe4es_glPushMatrix();
            fpe4es_glLoadIdentity();
            fpe4es_glMatrixMode(matmode);
            stipple_env = glstate->texenv[stipple_tmu].env.mode;
            fpe4es_glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
            stipple_tex2d = fpe4es_glIsEnabled(GL_TEXTURE_2D);
            stipple_alpha = fpe4es_glIsEnabled(GL_ALPHA_TEST);
            fpe4es_glEnable(GL_TEXTURE_2D);
            fpe4es_glEnable(GL_ALPHA_TEST);
            for (int k = 0; k < 4; k++) {
                stipple_texgen[k] = fpe4es_glIsEnabled(GL_TEXTURE_GEN_S + k);
                if (stipple_texgen[k]) fpe4es_glDisable(GL_TEXTURE_GEN_S + k);
            }
            stipple_afunc = glstate->alphafunc;
            stipple_aref = glstate->alpharef;
            fpe4es_glAlphaFunc(GL_GREATER, 0.0f);
            bind_stipple_tex();
            modeinit_t tmp;
            tmp.mode_init = list->mode_init;
            tmp.ilen = list->ilen ? list->ilen : list->len;
            list->tex[stipple_tmu] =
                gen_stipple_tex_coords(list->vert, list->indices, list->mode_inits ? list->mode_inits : &tmp,
        ...
        if (hardext.esversion == 1) {
            for (int a = 0; a < hardext.maxtex; a++) {
                if (glstate->enable.texture[a] || (stipple && a == stipple_tmu)) {
                    const GLint itarget =
                        (stipple && a == stipple_tmu) ? ENABLED_TEX2D : get_target(glstate->enable.texture[a]);
                    needclean[a] = 0;
                    use_texgen[a] = 0;
                    if ((glstate->enable.texgen_s[a] || glstate->enable.texgen_t[a] || glstate->enable.texgen_r[a] ||
                         glstate->enable.texgen_q[a])) {
                        TEXTURE(a);
                        RS(a, list->len);
                        gen_tex_coords(list->vert, list->normal, &glstate->texgened[a], list->len, &needclean[a], a,
                                       (list->ilen < list->len) ? indices : NULL,
                                       (list->ilen < list->len) ? list->ilen : 0);
                    } else if ((list->tex[a] == NULL) &&
                               !(list->mode == GL_POINT && glstate->texture.pscoordreplace[a])) {
                        RS(a, list->len);
                        gen_tex_coords(list->vert, list->normal, &glstate->texgened[a], list->len, &needclean[a], a,
                                       (list->ilen < list->len) ? indices : NULL,
                                       (list->ilen < list->len) ? list->ilen : 0);
                    ...
                    if ((list->tex[a] || (use_texgen[a] && !needclean[a])) &&
                        ((!(globals4es.texmat || glstate->texture_matrix[a]->identity)) || (bound->adjust))) {
                        if (!use_texgen[a]) {
                            RS(a, list->len);
                            if (list->tex_stride[a]) {
                                GLfloat* src = list->tex[a];
                                GLfloat* dst = glstate->texgened[a];
                                int stride = list->tex_stride[a] >>
                                             2; // stride need to be a multiple of 4 (i.e. sizeof(GLfloat))
                        ...
                        if (bound->adjust) {
                            tex_coord_npot(glstate->texgened[a], list->len, bound->width, bound->height, bound->nwidth,
                                           bound->nheight);
                        }
                    }
                }
                if ((list->tex[a] || (use_texgen[a] && !needclean[a])) /* && glstate->enable.texture[a]*/) {
                    ...
                    gles_glTexCoordPointer(4, GL_FLOAT, (use_texgen[a]) ? 0 : list->tex_stride[a],
                                           (use_texgen[a]) ? glstate->texgened[a] : list->tex[a]);
                } else {
                    if (glstate->gleshard->vertexattrib[ATT_MULTITEXCOORD0 + a].enabled ||
                        (hardext.esversion != 1)) { // optim for ES1.1 to avoid useless texture switch
                        TEXTURE(a);
                        fpe_glDisableClientState(GL_TEXTURE_COORD_ARRAY);
                    }
                    // else if (!glstate->enable.texgen_s[a] && glstate->enable.texture[a]) printf("LIBGL: texture[%i]
                    // without TexCoord, mode=0x%04X (init=0x%04X), listlen=%i\n", a, list->mode, list->mode_init,
                    // list->len);
                }
...
#undef TEXTURE
        ...
        if (use_vbo_array == 0) {
            if ((glstate->render_mode == GL_SELECT) || (glstate->polygon_mode == GL_LINE) ||
        ...
        if (use_vbo_array == 2) listActiveVBO(list, saved);
        if (list->use_vbo_array != use_vbo_array) list->use_vbo_array = use_vbo_array;
            ...
            if (glstate->render_mode == GL_SELECT) {
                ...
                if (glstate->polygon_mode == GL_LINE && list->mode_init >= GL_TRIANGLES) {
                    ...
                    if (!list->ind_lines) {
                        ...
                        tmp.mode_init = list->mode_init;
                        tmp.ilen = list->ilen;
                        int k = fill_lineIndices(list->mode_inits ? list->mode_inits : &tmp,
                                                 list->mode_inits ? list->mode_init_len : 1, list->mode, indices,
                                                 list->ind_lines);
                        list->ind_line = k;
                    }
                    ...
                    if (!use_vbo_indices) {
                        ...
                        gles_glBufferData(GL_ELEMENT_ARRAY_BUFFER, list->ilen * sizeof(GLushort), indices,
                                          GL_STATIC_DRAW);
                        use_vbo_indices = 2;
                        vbo_indices = 1;
            ...
            if (glstate->render_mode == GL_SELECT) {
                ...
                if ((glstate->polygon_mode == GL_LINE) && (list->mode_init >= GL_TRIANGLES)) {
                    if (!list->ind_lines) {
                        ...
                        tmp.ilen = len;
                        int k = fill_lineIndices(list->mode_inits ? list->mode_inits : &tmp,
                                                 list->mode_inits ? list->mode_init_len : 1, list->mode, NULL,
                                                 list->ind_lines);
                        list->ind_line = k;
                    }
                    bindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
        ...
        if (list->use_vbo_indices != use_vbo_indices) list->use_vbo_indices = use_vbo_indices;
        if (use_vbo_array == 2) listInactiveVBO(list, saved);
...
#undef TEXTURE
        ...
        if (stipple) {
            ...
            LOAD_GLES(glActiveTexture);
            if (glstate->gleshard->active != stipple_tmu) fpe4es_glActiveTexture(GL_TEXTURE0 + stipple_tmu);
            GLenum matmode;
            fpe4es_glGetIntegerv(GL_MATRIX_MODE, (GLint*)&matmode);
            fpe4es_glMatrixMode(GL_TEXTURE);
            fpe4es_glPopMatrix();
            fpe4es_glMatrixMode(matmode);
            fpe4es_glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, stipple_env);
            fpe4es_glAlphaFunc(stipple_afunc, stipple_aref);
            if (stipple_tex2d)
                fpe4es_glEnable(GL_TEXTURE_2D);
            else
                fpe4es_glDisable(GL_TEXTURE_2D);
            if (stipple_alpha)
                fpe4es_glEnable(GL_ALPHA_TEST);
            else
                fpe4es_glDisable(GL_ALPHA_TEST);
            for (int k = 0; k < 4; k++) {
                if (stipple_texgen[k]) fpe4es_glEnable(GL_TEXTURE_GEN_S + k);
            }
            if (glstate->gleshard->active != stipple_old) fpe4es_glActiveTexture(GL_TEXTURE0 + stipple_old);
        }
        if (list->post_color) fpe4es_glColor4fv(list->post_colors);
        if (list->post_normal) fpe4es_glNormal3fv(list->post_normals);
    } while ((list = list->next));
    fpe4es_glPopClientAttrib();
}
