...
#undef G2
        ...
        if (glstate->vao->vertexattrib[ATT_VERTEX].enabled) {
            if (glstate->vao->shared_arrays) {
                glstate->vao->vert.ptr =
                    (float*)copy_gl_pointer_tex(&glstate->vao->vertexattrib[ATT_VERTEX], 4, 0, count);
                list->vert = (float*)glstate->vao->vert.ptr + 4 * skip;
            } else
                list->vert = (float*)copy_gl_pointer_tex(&glstate->vao->vertexattrib[ATT_VERTEX], 4, skip, count);
        }
        if (glstate->vao->vertexattrib[ATT_COLOR].enabled) {
            if (glstate->vao->shared_arrays) {
                if (glstate->vao->vertexattrib[ATT_COLOR].size == GL_BGRA)
                    glstate->vao->color.ptr =
                        (float*)copy_gl_pointer_color_bgra(glstate->vao->vertexattrib[ATT_COLOR].pointer,
                                                           glstate->vao->vertexattrib[ATT_COLOR].stride, 4, 0, count);
                else
                    glstate->vao->color.ptr =
                        (float*)copy_gl_pointer_color(&glstate->vao->vertexattrib[ATT_COLOR], 4, 0, count);
                list->color = glstate->vao->color.ptr + 4 * skip;
            } else {
                if (glstate->vao->vertexattrib[ATT_COLOR].size == GL_BGRA)
                    list->color = (float*)copy_gl_pointer_color_bgra(glstate->vao->vertexattrib[ATT_COLOR].pointer,
                                                                     glstate->vao->vertexattrib[ATT_COLOR].stride, 4,
                                                                     skip, count);
                else
                    list->color = (float*)copy_gl_pointer_color(&glstate->vao->vertexattrib[ATT_COLOR], 4, skip, count);
            }
        }
        if (glstate->vao->vertexattrib[ATT_SECONDARY].enabled /* && glstate->enable.color_array*/) {
            if (glstate->vao->shared_arrays) {
                if (glstate->vao->vertexattrib[ATT_SECONDARY].size == GL_BGRA)
                    glstate->vao->secondary.ptr = (float*)copy_gl_pointer_color_bgra(
                    ...
                    glstate->vao->secondary.ptr =
                        (float*)copy_gl_pointer(&glstate->vao->vertexattrib[ATT_SECONDARY], 4, 0,
                                                count); // alpha chanel is always 0 for secondary...
                list->secondary = glstate->vao->secondary.ptr + 4 * skip;
            } else {
                if (glstate->vao->vertexattrib[ATT_SECONDARY].size == GL_BGRA)
                    list->secondary = (float*)copy_gl_pointer_color_bgra(
                        glstate->vao->vertexattrib[ATT_SECONDARY].pointer,
                        glstate->vao->vertexattrib[ATT_SECONDARY].stride, 4, skip, count);
                else
                    list->secondary = (float*)copy_gl_pointer(&glstate->vao->vertexattrib[ATT_SECONDARY], 4, skip,
                                                              count); // alpha chanel is always 0 for secondary...
            }
        }
        if (glstate->vao->vertexattrib[ATT_NORMAL].enabled) {
            if (glstate->vao->shared_arrays) {
                glstate->vao->normal.ptr =
                    (float*)copy_gl_pointer_raw(&glstate->vao->vertexattrib[ATT_NORMAL], 3, 0, count);
                list->normal = glstate->vao->normal.ptr + 3 * skip;
            } else
                list->normal = (float*)copy_gl_pointer_raw(&glstate->vao->vertexattrib[ATT_NORMAL], 3, skip, count);
        }
        if (glstate->vao->vertexattrib[ATT_FOGCOORD].enabled) {
            if (glstate->vao->shared_arrays) {
                glstate->vao->fog.ptr =
                    (float*)copy_gl_pointer_raw(&glstate->vao->vertexattrib[ATT_FOGCOORD], 1, 0, count);
                list->fogcoord = glstate->vao->fog.ptr + 1 * skip;
            } else
                list->fogcoord = (float*)copy_gl_pointer_raw(&glstate->vao->vertexattrib[ATT_FOGCOORD], 1, skip, count);
        }
        for (int i = 0; i < glstate->vao->maxtex; i++) {
            if (glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + i].enabled) {
                if (glstate->vao->shared_arrays) {
                    glstate->vao->tex[i].ptr =
                        (float*)copy_gl_pointer_tex(&glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + i], 4, 0, count);
                    list->tex[i] = glstate->vao->tex[i].ptr + 4 * skip;
                } else
                    list->tex[i] = (float*)copy_gl_pointer_tex(&glstate->vao->vertexattrib[ATT_MULTITEXCOORD0 + i], 4,
                                                               skip, count);
            }
        }
    }
...
if(count>500000) return;
    ...
    if (mode == GL_QUADS) {
        ...
        int ilen = (count * 3) / 2;
        if (iindices) {
            fpe4es_scratch(ilen * sizeof(GLuint));
            ...
            fpe4es_scratch(ilen * sizeof(GLushort));
    ...
    if (glstate->render_mode == GL_SELECT) {
        ...
        if (globals4es.usevbo == 3 && (glstate->vao->locked == 1 || glstate->vao->locked == 2)) {
        ...
        if (instancecount == 1) {
            if (!iindices && !sindices)
                fpe_glDrawArrays(mode, first, count);
            else
                fpe_glDrawElements(mode, count, (sindices) ? GL_UNSIGNED_SHORT : GL_UNSIGNED_INT,
                                   (sindices ? ((void*)sindices) : ((void*)iindices)));
        } else {
            if (!iindices && !sindices)
        ...
        for (int aa = 0; aa < 32; aa++) {
            if (!IS_TEX2D(glstate->enable.texture[aa]) && (IS_ANYTEX(glstate->enable.texture[aa]))) {
                fpe4es_glActiveTexture(GL_TEXTURE0 + aa);
                GLES.glActiveTexture(GL_TEXTURE0 + aa);
                realize_active();
                fpe_glDisable(GL_TEXTURE_2D);
            }
        }

        if (glstate->texture.client != old_tex) {
            fpe4es_glClientActiveTexture(old_tex + GL_TEXTURE0);
        }
    }
}
...
void APIENTRY_FPE4ES fpe4es_glDrawElements(GLenum mode, GLsizei count, GLenum type, const GLvoid* indices) {
    DBG(printf("glDrawElements(%s, %d, %s, %p), vtx=%p map=%p, pending=%d\n", PrintEnum(mode), count, PrintEnum(type),
               indices, (glstate->vao->vertex) ? glstate->vao->vertex->data : NULL,
...
void APIENTRY_FPE4ES fpe4es_glDrawArrays(GLenum mode, GLint first, GLsizei count) {
    ...
    if ((mode == GL_QUADS) && (count > 4 * 8000)) {
        ...
        for (int i = 0; i < count; i += 4 * 8000) {
            ...
            fpe4es_glDrawArrays(mode, i, cnt);
        }
        return;
    }
    noerrorShim();
...
void APIENTRY_FPE4ES fpe4es_glMultiDrawArrays(GLenum mode, const GLint* firsts, const GLsizei* counts,
                                              GLsizei primcount) {
    ...
    for (int i = 0; i < primcount; i++) {
        ...
        if (intercept) {
            if (list) {
                NewStage(list, STAGE_DRAW);
                if (globals4es.mergelist && list->stage >= STAGE_DRAW && is_list_compatible(list) &&
                    !list->use_glstate) {
                    list = NewDrawStage(list, mode);
...
void APIENTRY_FPE4ES fpe4es_glMultiDrawElements(GLenum mode, GLsizei* counts, GLenum type, const void* const* indices,
                                                GLsizei primcount) {
...
void APIENTRY_FPE4ES fpe4es_glDrawElementsInstanced(GLenum mode, GLsizei count, GLenum type, const void *indices,
GLsizei primcount) { DBG(printf("glDrawElementsInstanced(%s, %d, %s, %p, %d), list=%p pending=%d\n", PrintEnum(mode),
count, PrintEnum(type), indices, primcount, glstate->list.active, glstate->list.pending);) count = adjust_vertices(mode,
count);
    ...
    if(need_free) {
        sindices = copy_gl_array((glstate->vao->elements)?((void*)((char*)glstate->vao->elements->data +
(uintptr_t)indices)):indices, type, 1, 0, GL_UNSIGNED_SHORT, 1, 0, count, NULL); old_index = wantBufferIndex(0); } else
{ if(type==GL_UNSIGNED_INT) iindices = (glstate->vao->elements)?((void*)((char*)glstate->vao->elements->data +
(uintptr_t)indices)):(GLvoid*)indices; else sindices =
(glstate->vao->elements)?((void*)((char*)glstate->vao->elements->data + (uintptr_t)indices)):(GLvoid*)indices;
    }
...
AliasExport(void,glDrawElementsInstanced,,(GLenum mode, GLsizei count, GLenum type, const void *indices, GLsizei
primcount)); AliasExport(void,glDrawElementsInstanced,ARB,(GLenum mode, GLsizei count, GLenum type, const void *indices,
GLsizei primcount));*/
/*
void APIENTRY_FPE4ES fpe4es_glDrawElementsInstancedBaseVertex(GLenum mode, GLsizei count, GLenum type, const void
*indices, GLsizei primcount, GLint basevertex) { DBG(printf("glDrawElementsInstanceBaseVertex(%s, %d, %s, %p, %d, %d),
vtx=%p map=%p, pending=%d\n", PrintEnum(mode), count, PrintEnum(type), indices, primcount, basevertex,
(glstate->vao->vertex)?glstate->vao->vertex->data:NULL, (glstate->vao->elements)?glstate->vao->elements->data:NULL,
glstate->list.pending);) if(basevertex==0) fpe4es_glDrawElementsInstanced(mode, count, type, indices, primcount); else {
        count = adjust_vertices(mode, count);
        ...
        //BATCH Mode
        if(!compiling) {
            if(!intercept && glstate->list.pending && count>MAX_BATCH)    // too large and will not intercept, stop the
BATCH fpe4es_flush(); else if((!intercept && !glstate->list.pending && (count>=MIN_BATCH && count<=MAX_BATCH))
                || (intercept && globals4es.maxbatch)) {
                compiling = true;
                glstate->list.pending = 1;
                glstate->list.active = alloc_renderlist();
        ...
        if(type==GL_UNSIGNED_INT && hardext.elementuint && !compiling && !intercept)
            iindices = copy_gl_array((glstate->vao->elements)?(void*)((char*)glstate->vao->elements->data +
(uintptr_t)indices):indices, type, 1, 0, GL_UNSIGNED_INT, 1, 0, count, NULL); else sindices =
copy_gl_array((glstate->vao->elements)?(void*)((char*)glstate->vao->elements->data + (uintptr_t)indices):indices, type,
1, 0, GL_UNSIGNED_SHORT, 1, 0, count, NULL);
...
AliasExport(void,glDrawElementsInstancedBaseVertex,,(GLenum mode, GLsizei count, GLenum type, const void *indices,
GLsizei primcount, GLint basevertex)); AliasExport(void,glDrawElementsInstancedBaseVertex,ARB,(GLenum mode, GLsizei
count, GLenum type, const void *indices, GLsizei primcount, GLint basevertex));*/
