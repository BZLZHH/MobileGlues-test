...
EXPORT
void set_getmainfbsize(void(APIENTRY_FPE4ES* new_getMainFBSize)(int* w, int* h)) {
    fpe4es_getMainFBSize = (void*)new_getMainFBSize;
}

EXPORT
void set_getprocaddress(void*(APIENTRY_FPE4ES* new_proc_address)(const char*)) {
...
#define env(name, global, message)                                                                                     \
    ...
    int fpe4es_notest = !gles_getProcAddress;
#else
    int fpe4es_notest = IsEnvVarTrue("LIBGL_NOTEST");
#endif
    ...
    GetHardwareExtensions(fpe4es_notest);
    ...
    if (env_version) {
        snprintf(globals4es.version, 49, "%s fpe4es wrapper %d.%d.%d", env_version, MAJOR, MINOR, REVISION);
        SHUT_LOGD("Targeting OpenGL %s\n", env_version);
    } else {
        snprintf(globals4es.version, 49, "%d.%d fpe4es wrapper %d.%d.%d", globals4es.gl / 10, globals4es.gl % 10, MAJOR,
                 MINOR, REVISION);
...
#if defined(FPE4ES_COMPILE_FOR_USE_IN_SHARED_LIB) && defined(AMIGAOS4) // temporary workaround for not-working envs
...
void close_fpe4es() {
    ...
    fpe_writePSA();
    fpe_FreePSA();
#if defined(FPE4ES_COMPILE_FOR_USE_IN_SHARED_LIB) && defined(AMIGAOS4)
    os4CloseLib();
#endif
}
...
void (*const fpe4es_ctors[])() = {initialize_fpe4es, dll_init_done};
#pragma const_seg(".CRT$XTX")
void (*const fpe4es_dtor)() = close_fpe4es;
#pragma const_seg()
#endif
