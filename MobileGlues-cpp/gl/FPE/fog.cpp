...
void APIENTRY_FPE4ES fpe4es_glFogfv(GLenum pname, const GLfloat* params) {
    ...
    if (glstate->list.active)
        if (glstate->list.compiling) {
            ...
            fpe4es_flush();
    noerrorShim();
#define GO(A, name, size)                                                                                              \
    if (memcmp(A glstate->fog.name, params, size) == 0)                                                                \
        return;                                                                                                        \
    else                                                                                                               \
        memcpy(A glstate->fog.name, params, size);
#define GOI(name)                                                                                                      \
    if (glstate->fog.name == params[0])                                                                                \
        return;                                                                                                        \
    else                                                                                                               \
        glstate->fog.name = params[0];
    switch (pname) {
    case GL_FOG_MODE:
        GOI(mode)
        break;
