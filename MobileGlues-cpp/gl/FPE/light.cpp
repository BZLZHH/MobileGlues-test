...
void APIENTRY_FPE4ES fpe4es_glLightModelf(GLenum pname, GLfloat param) {
    ...
    if (glstate->list.active)
        if ((glstate->list.compiling)) {
            GLfloat dummy[4];
            dummy[0] = param;
            fpe4es_glLightModelfv(pname, dummy);
            return;
        } else
            fpe4es_flush();
    switch (pname) {
    case GL_LIGHT_MODEL_TWO_SIDE:
        ...
        if (glstate->fpe_state) glstate->fpe_state->twosided = param;
        break;
    case GL_LIGHT_MODEL_COLOR_CONTROL:
        if (param != GL_SINGLE_COLOR && param != GL_SEPARATE_SPECULAR_COLOR) {
            ...
            if (glstate->fpe_state) glstate->fpe_state->light_separate = value;
        }
    ...
    case GL_LIGHT_MODEL_LOCAL_VIEWER: {
        ...
        if (glstate->fpe_state) glstate->fpe_state->light_localviewer = value;
    }
        return; // NOT Supported in GLES 1.1 anyway
...
void APIENTRY_FPE4ES fpe4es_glLightModelfv(GLenum pname, const GLfloat* params) {
    // printf("%sglLightModelfv(%04X, [%.2f, %.2f, %.2f, %.2f])\n", (state.list.compiling)?"list":"", pname, params[0],
    ...
    case GL_LIGHT_MODEL_TWO_SIDE:
        ...
        if (glstate->fpe_state) glstate->fpe_state->twosided = params[0];
        break;
    case GL_LIGHT_MODEL_COLOR_CONTROL:
...
void APIENTRY_FPE4ES fpe4es_glLightfv(GLenum light, GLenum pname, const GLfloat* params) {
    // printf("%sglLightfv(%s, %s, %p=[%.2f, %.2f, %.2f, %.2f])\n", (glstate->list.compiling)?"list":"",
    // PrintEnum(light), PrintEnum(pname), params, (params)?params[0]:0.0f, (params)?params[1]:0.0f,
    // (params)?params[2]:0.0f, (params)?params[3]:0.0f);
    ...
    switch (pname) {
    case GL_AMBIENT:
        if (memcmp(glstate->light.lights[nl].ambient, params, 4 * sizeof(GLfloat)) == 0) return;
        memcpy(glstate->light.lights[nl].ambient, params, 4 * sizeof(GLfloat));
        break;
    case GL_DIFFUSE:
        if (memcmp(glstate->light.lights[nl].diffuse, params, 4 * sizeof(GLfloat)) == 0) return;
        memcpy(glstate->light.lights[nl].diffuse, params, 4 * sizeof(GLfloat));
        break;
    case GL_SPECULAR:
        if (memcmp(glstate->light.lights[nl].specular, params, 4 * sizeof(GLfloat)) == 0) return;
        memcpy(glstate->light.lights[nl].specular, params, 4 * sizeof(GLfloat));
        break;
    case GL_POSITION:
        vector_matrix(params, getMVMat(), tmp);
        if (memcmp(glstate->light.lights[nl].position, tmp, 4 * sizeof(GLfloat)) == 0) return;
    ...
    case GL_SPOT_DIRECTION:
        ...
        if (memcmp(glstate->light.lights[nl].spotDirection, tmp, 3 * sizeof(GLfloat)) == 0) return;
    ...
    case GL_SPOT_EXPONENT:
        ...
        if (glstate->light.lights[nl].spotExponent == params[0]) return;
    ...
    case GL_SPOT_CUTOFF:
        ...
        if (glstate->light.lights[nl].spotCutoff == params[0]) return;
    ...
    case GL_CONSTANT_ATTENUATION:
        ...
        if (glstate->light.lights[nl].constantAttenuation == params[0]) return;
    ...
    case GL_LINEAR_ATTENUATION:
        ...
        if (glstate->light.lights[nl].linearAttenuation == params[0]) return;
    ...
    case GL_QUADRATIC_ATTENUATION:
        ...
        if (glstate->light.lights[nl].quadraticAttenuation == params[0]) return;
...
void APIENTRY_FPE4ES fpe4es_glLightf(GLenum light, GLenum pname, GLfloat param) {
    GLfloat dummy[4];
    dummy[0] = param;
    fpe4es_glLightfv(light, pname, dummy);
}
...
void APIENTRY_FPE4ES fpe4es_glMaterialfv(GLenum face, GLenum pname, const GLfloat* params) {
    ...
    case GL_SHININESS:
        ...
        if (face == GL_FRONT_AND_BACK || face == GL_FRONT) {
            ...
            if (glstate->fpe_state) glstate->fpe_state->cm_front_nullexp = (*params <= 0.0) ? 0 : 1;
        }
        if (face == GL_FRONT_AND_BACK || face == GL_BACK) {
            glstate->material.back.shininess = *params;
            if (glstate->fpe_state) glstate->fpe_state->cm_back_nullexp = (*params <= 0.0) ? 0 : 1;
        }
        break;
...
void APIENTRY_FPE4ES fpe4es_glMaterialf(GLenum face, GLenum pname, GLfloat param) {
    ERROR_IN_BEGIN
    if (glstate->list.active)
        if (glstate->list.compiling) {
            GLfloat params[4];
    ...
    if (face == GL_FRONT_AND_BACK || face == GL_FRONT) {
        if (glstate->material.front.shininess == param) return;
        glstate->material.front.shininess = param;
    }
    if (face == GL_FRONT_AND_BACK || face == GL_BACK) {
        if (glstate->material.back.shininess == param) return;
...
void APIENTRY_FPE4ES fpe4es_glColorMaterial(GLenum face, GLenum mode) {
    ...
    if (face == GL_FRONT_AND_BACK || face == GL_FRONT) glstate->material.front.colormat = mode;
    if (face == GL_FRONT_AND_BACK || face == GL_BACK) glstate->material.back.colormat = mode;
    if (glstate->fpe_state) {
