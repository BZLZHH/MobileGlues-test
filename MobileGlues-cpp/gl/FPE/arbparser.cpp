...
#define IS_SWIZ_VALUE(v)                                                                                               \
    ((((v) >= 'w') && ((v) <= 'z')) || ((v) == 'r') || ((v) == 'g') || ((v) == 'b') || ((v) == 'a'))
#define IS_SWIZZLE(str)                                                                                                \
    (IS_SWIZ_VALUE((str)[0]) &&                                                                                        \
     (((str)[1] == '\0') ||                                                                                            \
      (IS_SWIZ_VALUE((str)[1]) &&                                                                                      \
       (((str)[2] == '\0') ||                                                                                          \
        (IS_SWIZ_VALUE((str)[2]) && (((str)[3] == '\0') || (IS_SWIZ_VALUE((str)[3]) && ((str)[4] == '\0'))))))))
...
eToken readNextToken(sCurStatus* curStatus) {
    ...
    case '9':
        ...
        if ((*curStatus->endOfToken == 'e') || (*curStatus->endOfToken == 'E')) {
            ...
            while (e-- != 0)
                curStatus->tokFloat *= s ? 10.f : 0.1f;
        }
...
int resolveAttrib(sCurStatus_NewVar* newVar, int vertex) {
    char* tok = (char*)popFIFO((sArray*)newVar);
    ...
    if (!tok) {
        ...
        free(tok);
        tok = (char*)popFIFO((sArray*)newVar);
        if (!tok) {
            ...
            if (!IS_NONE_OR_SWIZZLE) {
                ...
                if ((tok[0] == '[') && (newVar->strLen >= 2) && (newVar->strParts[1][0] == ']') &&
                    ...
                    free(tok);
                    char* tex = (char*)popFIFO((sArray*)newVar);
            ...
            if (!tok) {
                ...
                return 1;
                       ...
                       (newVar->strParts[0][0] >= '0') && (newVar->strParts[0][0] <= '9')) {
                free(tok);
                char* attr = (char*)popFIFO((sArray*)newVar);
                free(popFIFO((sArray*)newVar));
...
int resolveOutput(sCurStatus_NewVar* newVar, int vertex, struct sSpecialCases* specialCases) {
    ...
    if (!tok) {
        ...
        if (!tok) {
            ...
            pushArray((sArray*)&newVar->var->init, strdup("fpe4es_FogFragCoordTemp"));
            newVar->var->init.strings_total_len = 22;
        ...
        if (!tok) {
            ...
            pushArray((sArray*)&newVar->var->init, strdup("fpe4es_FragDepthTemp"));
            newVar->var->init.strings_total_len = 12;
        } else {
            ARBCONV_DBG_RE("Failed to get output: result.%s\n", tok)
            free(tok);
...
char** resolveParam(sCurStatus_NewVar* newVar, int vertex, int type) {
    ...
    if (!tok) {
        ...
        if (!tok) {
            ...
            if (newVar->strParts[0][0] == '[') {
                free(popFIFO((sArray*)newVar));
                char* sln = (char*)popFIFO((sArray*)newVar);
            ...
            if (!tok) {
                    ...
                    if (newVar->strLen && !IS_NEW_STR_OR_SWIZZLE(newVar->strParts[0], type)) {
                        ...
                        if (!strcmp(tok, "row") &&
                            ...
                            if ((newVar->strLen >= 3) && (newVar->strParts[0][0] == '.') &&
                                (newVar->strParts[0][1] == '.') && (newVar->strParts[1][0] >= '0') &&
                ...
                if (newVar->strLen && !IS_NEW_STR_OR_SWIZZLE(newVar->strParts[0], type)) {
                    ...
                    if (newVar->strLen && !IS_NEW_STR_OR_SWIZZLE(newVar->strParts[0], type)) {
                        tok = (char*)popFIFO((sArray*)newVar);
                        if (!strcmp(tok, "row") && (newVar->strLen >= 3) && (newVar->strParts[0][0] == '[') &&
                            (newVar->strParts[1][0] >= '0') && (newVar->strParts[1][0] <= '9')) {
                            free(tok);
                            free(popFIFO((sArray*)newVar));
                            tok = (char*)popFIFO((sArray*)newVar);
                ...
                if (newVar->strLen && !IS_NEW_STR_OR_SWIZZLE(newVar->strParts[0], type)) {
                    ...
                    if (newVar->strLen && !IS_NEW_STR_OR_SWIZZLE(newVar->strParts[0], type)) {
                        tok = (char*)popFIFO((sArray*)newVar);
                        if (!strcmp(tok, "row") && newVar->strLen && (newVar->strParts[0][0] >= '0') &&
                    ...
                    matrixNameMallocd = (char*)malloc((mtxNameLen + 1) * sizeof(char));
        ...
        pseudoSt.outputString = (char*)malloc(DEFAULT_STRING_MALLOC_SIZE);
     ...
     * <paramSingleItemDecl>
     ...
     "]"
     \ ? "state" "." "matrix" "." "palette" "[" <statePaletteMatNum> "]" "." "transpose" "." "row" "["
     <stateMatrixRowNum> "]"
     \ ? "state" "." "matrix" "." "palette" "[" <statePaletteMatNum> "]" "." "invtrans" "." "row" "["
     <stateMatrixRowNum> "]"
     ...
     "]"
     \ ? "state" "." "matrix" "." "program" "[" <stateProgramMatNum> "]" "." "transpose" "." "row" "["
     <stateMatrixRowNum> "]"
     \ ? "state" "." "matrix" "." "program" "[" <stateProgramMatNum> "]" "." "invtrans" "." "row" "["
     <stateMatrixRowNum> "]"
...
void parseToken(sCurStatus* curStatusPtr, int vertex, char** error_msg, struct sSpecialCases* specialCases) {
    ...
    case ST_LINE_START:
        ...
        case TOK_IDENTIFIER: {
            ...
            if ((inst = STR2INST(tok, &curStatusPtr->curValue.newInst.inst.saturated)) != INST_UNK) {
                ...
                curStatusPtr->valueType = _sCurStatus::TYPE_INST_DECL;
                ...
                for (int i = 0; i < MAX_OPERANDS; ++i) {
                    ...
                    curStatusPtr->curValue.newInst.inst.vars[i].swizzle[0] = _sInstruction_Vars::SWIZ_NONE;
                    curStatusPtr->curValue.newInst.inst.vars[i].swizzle[1] = _sInstruction_Vars::SWIZ_NONE;
                    curStatusPtr->curValue.newInst.inst.vars[i].swizzle[2] = _sInstruction_Vars::SWIZ_NONE;
                    curStatusPtr->curValue.newInst.inst.vars[i].swizzle[3] = _sInstruction_Vars::SWIZ_NONE;
                }
                ...
                if ((vtype == VARTYPE_ADDRESS) && !vertex) {
                    ...
                    curStatusPtr->valueType = _sCurStatus::_sCurStatus::TYPE_ALIAS_DECL;
                    curStatusPtr->curValue.string = NULL;
                } else {
                    curStatusPtr->status = ST_VARIABLE;
                    curStatusPtr->valueType = _sCurStatus::TYPE_VARIABLE_DECL;
                ...
                curStatusPtr->valueType = _sCurStatus::TYPE_OPTION_DECL;
        ...
        case VARTYPE_TEMP:
            ...
            case TOK_IDENTIFIER: {
                ...
                if (!strcmp(tok, "half")) {
                    ...
                    pushArray((sArray*)curStatusPtr->curValue.newVar.var, strdup("fpe4es_half"));
            ...
            case TOK_COMMA: {
                ...
                int ret;
                khint_t varIdx =
                    kh_put(variables, curStatusPtr->varsMap, curStatusPtr->curValue.newVar.var->names[0], &ret);
                if (ret < 0) {
                    FAIL("Unknown error");
                }
                kh_val(curStatusPtr->varsMap, varIdx) = curStatusPtr->curValue.newVar.var;
            ...
            case TOK_END_OF_INST: {
                ...
                kh_val(curStatusPtr->varsMap, varIdx) = curStatusPtr->curValue.newVar.var;
                pushArray((sArray*)&curStatusPtr->variables, curStatusPtr->curValue.newVar.var);
                curStatusPtr->valueType = _sCurStatus::_sCurStatus::TYPE_NONE;
                curStatusPtr->status = ST_LINE_START;
            } break;
    ...
    case ST_VARIABLE_INIT:
        ...
        case VARTYPE_ATTRIB:
            ...
            case TOK_END_OF_INST: {
                ...
                if ((tok = (char*)popFIFO((sArray*)&curStatusPtr->curValue.newVar))) {
                    free(tok);
                    FAIL("Not a valid attribute");
                }
        ...
        case VARTYPE_PARAM:
            ...
            case TOK_INTEGER:
                ...
                if ((curStatusPtr->curValue.newVar.state > 10) || (curStatusPtr->curValue.newVar.state % 2)) {
                    FAIL("Invalid state");
                }
        ...
        case VARTYPE_PARAM_MULT:
            ...
            case TOK_END_OF_INST: {
                ...
                } while (curStatusPtr->curValue.newVar.strLen && (curStatusPtr->curValue.newVar.strParts[0][0] == ','));
                ...
                pushArray((sArray*)&curStatusPtr->variables, curStatusPtr->curValue.newVar.var);
                freeArray((sArray*)&curStatusPtr->curValue.newVar);
                curStatusPtr->valueType = _sCurStatus::TYPE_NONE;
                curStatusPtr->status = ST_LINE_START;
            } break;
    ...
    case ST_ALIASING:
        switch (curStatusPtr->curToken) {
        case TOK_IDENTIFIER: {
            if (curStatusPtr->valueType != _sCurStatus::TYPE_ALIAS_DECL) {
                // Already aliased
        ...
        case TOK_END_OF_INST:
            if (curStatusPtr->valueType == _sCurStatus::TYPE_NONE) {
                curStatusPtr->status = ST_LINE_START;
            } else {
...
#define STATE_AFTER_SWIZZLE -1
        ...
        sInstruction_Vars* curVarPtr =
            (sInstruction_Vars*)&curStatusPtr->curValue.newInst.inst.vars[curStatusPtr->curValue.newInst.curArg];
        int texSampler =
            INSTTEX(curStatusPtr->curValue.newInst.inst.type) && (curStatusPtr->curValue.newInst.curArg == 3);
        switch (curStatusPtr->curToken) {
        case TOK_WHITESPACE:
        case TOK_NEWLINE:
            ...
            case STATE_AFTER_VALID_LSQBR_SIGN: {
                char* faa =
                    (char*)realloc(curVarPtr->floatArrAddr,
        ...
        case TOK_IDENTIFIER: {
            ...
            case STATE_AFTER_VALID_DOT: {
                ...
                for (int i = 0; !e && (tok[i] != '\0'); ++i) {
                    ...
                    case 'r':
                    case 'x':
                        curVarPtr->swizzle[i] = _sInstruction_Vars::SWIZ_X;
                        break;
                    case 'g':
                    case 'y':
                        curVarPtr->swizzle[i] = _sInstruction_Vars::SWIZ_Y;
                        break;
                    case 'b':
                    case 'z':
                        curVarPtr->swizzle[i] = _sInstruction_Vars::SWIZ_Z;
                        break;
                    case 'a':
                    case 'w':
                        curVarPtr->swizzle[i] = _sInstruction_Vars::SWIZ_W;
                        break;
                    default:
                        e = 1;
                        break;
        ...
        case TOK_COMMA:
            ...
            case STATE_RBRACE:       // directly to the resolveParam part...
                if (INSTTEX(curStatusPtr->curValue.newInst.inst.type) && (curStatusPtr->curValue.newInst.curArg == 2)) {
                    ...
                    if (curStatusPtr->_fixedNewVar.strLen) {
                        ...
                        // Swizzle
                        int e = 0;
                        char* swiz = (char*)popArray((sArray*)&curStatusPtr->_fixedNewVar);
            ...
            case STATE_AFTER_SWIZZLE:
                if (curStatusPtr->curToken == TOK_COMMA) {
                    ...
                    pushArray((sArray*)&curStatusPtr->instructions,
