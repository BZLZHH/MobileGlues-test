...
#define write_each(amod, vmod, hmod)                                                                                   \
    ...
    switch (src_type) {
    type_case(GL_DOUBLE, GLdouble, read_each(, , )) type_case(GL_FLOAT, GLfloat, read_each(, , ))
...
bool pixel_convert(const GLvoid* src, GLvoid** dst, GLuint width, GLuint height, GLenum src_format, GLenum src_type,
    ...
    // printf("pixel conversion: %ix%i - %s, %s (%d) ==> %s, %s (%d), transform=%i, align=%d, src_width=%d(%d),
    // dst_width=%d(%d)\n", width, height, PrintEnum(src_format), PrintEnum(src_type),pixel_sizeof(src_format,
    // src_type), PrintEnum(dst_format), PrintEnum(dst_type), pixel_sizeof(dst_format, dst_type),
    // raster_need_transform(), align, src_width, src_widthadj, dst_width2, dst_width);
...
bool pixel_halfscale(const GLvoid* old, GLvoid** new, GLuint width, GLuint height, GLenum format, GLenum type) {
    ...
    // printf("LIBGL: halfscaling %ux%u -> %ux%u (%s / %s)\n", width, height, new_width, new_height, PrintEnum(format),
    // PrintEnum(type));
    const colorlayout_t* src_color;
...
bool pixel_doublescale(const GLvoid* old, GLvoid** new, GLuint width, GLuint height, GLenum format, GLenum type) {
    ...
    for (int y = 0; y + 1 < new_height; y += 2) {
        for (int x = 0; x + 1 < new_width; x += 2) {
            pix0 = src + ((x / 2) + (y / 2) * width) * pixel_size;
            memcpy((void*)pos, (void*)pix0, pixel_size);
