...
void APIENTRY_FPE4ES fpe_ReleventState_DefaultVertex(fpe_state_t* dest, fpe_state_t* src, shaderconv_need_t* need) {
...
void APIENTRY_FPE4ES fpe_ReleventState(fpe_state_t* dest, fpe_state_t* src, int fixed) {
    // filter out some non relevant state (like texture stuff if texture is disabled)
    memcpy(dest, src, sizeof(fpe_state_t));
    // alpha test
    if (!dest->alphatest) {
...
int APIENTRY_FPE4ES fpe_IsEmpty(fpe_state_t* state) {
...
void APIENTRY_FPE4ES fpe_oldprogram(fpe_state_t* state) {
    ...
    glstate->fpe->vert = fpe4es_glCreateShader(GL_VERTEX_SHADER);
    if (state->vertex_prg_id) {
        fpe4es_glShaderSource(glstate->fpe->vert, 1,
                              fpe_CustomVertexShader(old_vtx->shader->source, state, state->fragment_prg_id ? 0 : 1),
                              NULL);
        fpe4es_glCompileShader(glstate->fpe->vert);
        fpe4es_glGetShaderiv(glstate->fpe->vert, GL_COMPILE_STATUS, &status);
        if (status != GL_TRUE) {
            char buff[1000];
        ...
        fpe4es_glShaderSource(glstate->fpe->vert, 1, fpe_VertexShader(&old_frg->shader->need, state), NULL);
        fpe4es_glCompileShader(glstate->fpe->vert);
        fpe4es_glGetShaderiv(glstate->fpe->vert, GL_COMPILE_STATUS, &status);
        if (status != GL_TRUE) {
            char buff[1000];
    ...
    fpe4es_glAttachShader(glstate->fpe->prog, glstate->fpe->vert);
    glstate->fpe->frag = fpe4es_glCreateShader(GL_FRAGMENT_SHADER);
    if (state->fragment_prg_id) {
        fpe4es_glShaderSource(glstate->fpe->frag, 1, fpe_CustomFragmentShader(old_frg->shader->source, state), NULL);
        fpe4es_glCompileShader(glstate->fpe->frag);
        fpe4es_glGetShaderiv(glstate->fpe->frag, GL_COMPILE_STATUS, &status);
        if (status != GL_TRUE) {
            char buff[1000];
        ...
        fpe4es_glShaderSource(glstate->fpe->frag, 1, fpe_FragmentShader(&old_vtx->shader->need, state), NULL);
        fpe4es_glCompileShader(glstate->fpe->frag);
        fpe4es_glGetShaderiv(glstate->fpe->frag, GL_COMPILE_STATUS, &status);
        if (status != GL_TRUE) {
            char buff[1000];
    ...
    fpe4es_glAttachShader(glstate->fpe->prog, glstate->fpe->frag);
    // Ok, and now link the program
    fpe4es_glLinkProgram(glstate->fpe->prog);
    fpe4es_glGetProgramiv(glstate->fpe->prog, GL_LINK_STATUS, &status);
    if (status != GL_TRUE) {
        char buff[1000];
...
void APIENTRY_FPE4ES fpe_program(int ispoint) {
    ...
    if (glstate->fpe->glprogram == NULL) {
        glstate->fpe->prog = fpe4es_glCreateProgram();
        DBG(int from_psa = 1;)
        if (fpe_GetProgramPSA(glstate->fpe->prog, &state) == 0) {
            ...
            if (state.vertex_prg_id || state.fragment_prg_id) {
                ...
                glstate->fpe->vert = fpe4es_glCreateShader(GL_VERTEX_SHADER);
                fpe4es_glShaderSource(glstate->fpe->vert, 1, fpe_VertexShader(NULL, glstate->fpe_state), NULL);
                fpe4es_glCompileShader(glstate->fpe->vert);
                fpe4es_glGetShaderiv(glstate->fpe->vert, GL_COMPILE_STATUS, &status);
                if (status != GL_TRUE) {
                    char buff[1000];
                ...
                glstate->fpe->frag = fpe4es_glCreateShader(GL_FRAGMENT_SHADER);
                fpe4es_glShaderSource(glstate->fpe->frag, 1, fpe_FragmentShader(NULL, glstate->fpe_state), NULL);
                fpe4es_glCompileShader(glstate->fpe->frag);
                fpe4es_glGetShaderiv(glstate->fpe->frag, GL_COMPILE_STATUS, &status);
                if (status != GL_TRUE) {
                    char buff[1000];
...
program_t* APIENTRY_FPE4ES fpe_CustomShader(program_t* glprogram, fpe_state_t* state) {
    ...
    if (fpe->glprogram == NULL) {
        GLint status;
        fpe->vert = fpe4es_glCreateShader(GL_VERTEX_SHADER);
        fpe4es_glShaderSource(fpe->vert, 1, fpe_CustomVertexShader(glprogram->last_vert->source, state, 0), NULL);
        fpe4es_glCompileShader(fpe->vert);
        fpe4es_glGetShaderiv(fpe->vert, GL_COMPILE_STATUS, &status);
        if (status != GL_TRUE) {
            char buff[1000];
            fpe4es_glGetShaderInfoLog(fpe->vert, 1000, NULL, buff);
            printf("LIBGL: FPE Custom Vertex shader compile failed: %s\n", buff);
            return glprogram; // fallback to non-customized custom program..
        }
        fpe->frag = fpe4es_glCreateShader(GL_FRAGMENT_SHADER);
        fpe4es_glShaderSource(fpe->frag, 1, fpe_CustomFragmentShader(glprogram->last_frag->source, state), NULL);
        fpe4es_glCompileShader(fpe->frag);
        fpe4es_glGetShaderiv(fpe->frag, GL_COMPILE_STATUS, &status);
        if (status != GL_TRUE) {
            char buff[1000];
            fpe4es_glGetShaderInfoLog(fpe->frag, 1000, NULL, buff);
            printf("LIBGL: FPE Custom Fragment shader compile failed: %s\n", buff);
            return glprogram; // fallback to non-customized custom program..
        }
        fpe->prog = fpe4es_glCreateProgram();
        fpe4es_glAttachShader(fpe->prog, fpe->vert);
        fpe4es_glAttachShader(fpe->prog, fpe->frag);
        ...
        fpe4es_glLinkProgram(fpe->prog);
        fpe4es_glGetProgramiv(fpe->prog, GL_LINK_STATUS, &status);
        if (status != GL_TRUE) {
            char buff[1000];
            fpe4es_glGetProgramInfoLog(fpe->prog, 1000, NULL, buff);
...
program_t* APIENTRY_FPE4ES fpe_CustomShader_DefaultVertex(program_t* glprogram, fpe_state_t* state_vertex) {
    // state is not empty and glprogram already has some cache (it may be empty, but kh'thingy is initialized)
    // TODO: what if program is composed of more then 1 vertex or fragment shader?
    ...
    if (fpe->glprogram == NULL) {
        GLint status;
        fpe->vert = fpe4es_glCreateShader(GL_VERTEX_SHADER);
        fpe4es_glShaderSource(fpe->vert, 1, fpe_VertexShader(glprogram->default_need, state_vertex), NULL);
        fpe4es_glCompileShader(fpe->vert);
        fpe4es_glGetShaderiv(fpe->vert, GL_COMPILE_STATUS, &status);
        if (status != GL_TRUE) {
            char buff[1000];
        ...
        fpe->frag = fpe4es_glCreateShader(GL_FRAGMENT_SHADER);
        fpe4es_glShaderSource(fpe->frag, 1, fpe_CustomFragmentShader(glprogram->last_frag->source, state_vertex), NULL);
        fpe4es_glCompileShader(fpe->frag);
        fpe4es_glGetShaderiv(fpe->frag, GL_COMPILE_STATUS, &status);
        if (status != GL_TRUE) {
            char buff[1000];
...
void APIENTRY_FPE4ES fpe_SyncUniforms(uniformcache_t* cache, program_t* glprogram) {
    // TODO: Optimize this...
...
void APIENTRY_FPE4ES fpe_glClientActiveTexture(GLenum texture) {
    DBG(printf("fpe_glClientActiveTexture(%s)\n", PrintEnum(texture));)
}

void APIENTRY_FPE4ES fpe_EnableDisableClientState(GLenum cap, GLboolean val) {
    int att = -1;
    switch (cap) {
...
void APIENTRY_FPE4ES fpe_glEnableClientState(GLenum cap) {
    DBG(printf("fpe_glEnableClientState(%s)\n", PrintEnum(cap));)
    fpe_EnableDisableClientState(cap, GL_TRUE);
}

void APIENTRY_FPE4ES fpe_glDisableClientState(GLenum cap) {
    DBG(printf("fpe_glDisableClientState(%s)\n", PrintEnum(cap));)
    fpe_EnableDisableClientState(cap, GL_FALSE);
}

void APIENTRY_FPE4ES fpe_glMultiTexCoord4f(GLenum target, GLfloat s, GLfloat t, GLfloat r, GLfloat q) {}

void APIENTRY_FPE4ES fpe_glSecondaryColorPointer(GLint size, GLenum type, GLsizei stride, const GLvoid* pointer) {
...
void APIENTRY_FPE4ES fpe_glVertexPointer(GLint size, GLenum type, GLsizei stride, const GLvoid* pointer) {
...
void APIENTRY_FPE4ES fpe_glColorPointer(GLint size, GLenum type, GLsizei stride, const GLvoid* pointer) {
...
void APIENTRY_FPE4ES fpe_glNormalPointer(GLenum type, GLsizei stride, const GLvoid* pointer) {
...
void APIENTRY_FPE4ES fpe_glTexCoordPointer(GLint size, GLenum type, GLsizei stride, const GLvoid* pointer) {
    fpe_glTexCoordPointerTMU(size, type, stride, pointer, glstate->texture.client);
}

void APIENTRY_FPE4ES fpe_glTexCoordPointerTMU(GLint size, GLenum type, GLsizei stride, const GLvoid* pointer, int TMU) {
...
void APIENTRY_FPE4ES fpe_glFogCoordPointer(GLenum type, GLsizei stride, const GLvoid* pointer) {
...
void APIENTRY_FPE4ES fpe_glEnable(GLenum cap) {
    fpe4es_glEnable(cap); // may reset fpe curent program
}
void APIENTRY_FPE4ES fpe_glDisable(GLenum cap) {
    fpe4es_glDisable(cap); // may reset fpe curent program
}

void APIENTRY_FPE4ES fpe_glColor4f(GLfloat red, GLfloat green, GLfloat blue, GLfloat alpha) {
    noerrorShim();
}

void APIENTRY_FPE4ES fpe_glNormal3f(GLfloat nx, GLfloat ny, GLfloat nz) {
    noerrorShim();
}

void APIENTRY_FPE4ES fpe_glDrawArrays(GLenum mode, GLint first, GLsizei count) {
...
void APIENTRY_FPE4ES fpe_glDrawElements(GLenum mode, GLsizei count, GLenum type, const GLvoid* indices) {
...
void APIENTRY_FPE4ES fpe_glDrawArraysInstanced(GLenum mode, GLint first, GLsizei count, GLsizei primcount) {
...
void APIENTRY_FPE4ES fpe_glDrawElementsInstanced(GLenum mode, GLsizei count, GLenum type, const GLvoid* indices,
                                                 GLsizei primcount) {
    DBG(printf("fpe_glDrawElementsInstanced(%s, %d, %s, %p, %d), program=%d\n", PrintEnum(mode), count, PrintEnum(type),
               indices, primcount, glstate->glsl->program);)
    LOAD_GLES(glDrawElements);
    LOAD_GLES2(glVertexAttrib4fv);
    scratch_t scratch = {0};
    realize_glenv(mode == GL_POINTS, 0, count, type, indices, &scratch);
...
void APIENTRY_FPE4ES fpe_glMatrixMode(GLenum mode) {
    noerrorShim();
}

void APIENTRY_FPE4ES fpe_glLightModelf(GLenum pname, GLfloat param) {
    noerrorShim();
}
void APIENTRY_FPE4ES fpe_glLightModelfv(GLenum pname, const GLfloat* params) {
    noerrorShim();
}
void APIENTRY_FPE4ES fpe_glLightfv(GLenum light, GLenum pname, const GLfloat* params) {
    noerrorShim();
}
void APIENTRY_FPE4ES fpe_glMaterialfv(GLenum face, GLenum pname, const GLfloat* params) {
    noerrorShim();
}
void APIENTRY_FPE4ES fpe_glMaterialf(GLenum face, GLenum pname, const GLfloat param) {
...
void APIENTRY_FPE4ES fpe_glFogfv(GLenum pname, const GLfloat* params) {
    noerrorShim();
...
void APIENTRY_FPE4ES fpe_glPointParameterfv(GLenum pname, const GLfloat* params) {
    noerrorShim();
}
void APIENTRY_FPE4ES fpe_glPointSize(GLfloat size) {
    noerrorShim();
}

void APIENTRY_FPE4ES fpe_glAlphaFunc(GLenum func, GLclampf ref) {
    noerrorShim();
    int f = FPE_ALWAYS;
    switch (func) {
    case GL_NEVER:
...
void realize_glenv(int ispoint, int first, int count, GLenum type, const void* indices, scratch_t* scratch) {
    ...
    if (fpe4es_glIsProgram(glstate->glsl->program)) {
        ...
        if (glstate->gleshard->program != program) {
            ...
            if (fpe4es_glIsProgram(glstate->gleshard->program)) {
...
const char* fpe4es_code = "_fpe4es_";
const char* lightsource_code = "_fpe4es_LightSource[";
const char* lightsource_fpe_code = "_fpe4es_LightSource_";
const char* lightmodel_code = "_fpe4es_LightModel.";
const char* frontmaterial_code = "_fpe4es_FrontMaterial";
const char* backmaterial_code = "_fpe4es_BackMaterial";
const char* frontmaterial_fpe_code = "_fpe4es_FrontMaterial_shininess";
const char* backmaterial_fpe_code = "_fpe4es_BackMaterial_shininess";
const char* frontlightmodelprod_code = "_fpe4es_FrontLightModelProduct";
const char* backlightmodelprod_code = "_fpe4es_BackLightModelProduct";
const char* frontlightprod_code = "_fpe4es_FrontLightProduct[";
const char* backlightprod_code = "_fpe4es_BackLightProduct[";
const char* frontlightprod_fpe_code = "_fpe4es_FrontLightProduct_";
const char* backlightprod_fpe_code = "_fpe4es_BackLightProduct_";
const char* normalrescale_code = "_fpe4es_NormalScale";
const char* instanceID_code = "_fpe4es_InstanceID";
const char* clipplanes_code = "_fpe4es_ClipPlane[";
const char* clipplanes_fpe_code = "_fpe4es_ClipPlane_";
const char* point_code = "_fpe4es_Point";
const char* texenvcolor_code = "_fpe4es_TextureEnvColor[";
const char* texenvcolor_fpe_code = "_fpe4es_TextureEnvColor_";
const char* texenvcolor_noa_code = "_fpe4es_TextureEnvColor";
const char* texgeneyestart_code = "_fpe4es_EyePlane";
const char* texgeneye_code = "_fpe4es_EyePlane%c[";
const char* texgeneye_fpe_code = "_fpe4es_EyePlane%c_";
const char* texgeneye_noa_code = "_fpe4es_EyePlane%c";
const char* texgenobjstart_code = "_fpe4es_ObjectPlane";
const char* texgenobj_code = "_fpe4es_ObjectPlane%c[";
const char* texgenobj_fpe_code = "_fpe4es_ObjectPlane%c_";
const char* texgenobj_noa_code = "_fpe4es_ObjectPlane%c";
const char texgenCoords[4] = {'S', 'T', 'R', 'Q'};
const char* alpharef_code = "_fpe4es_AlphaRef";
const char* fpetexSampler_code = "_fpe4es_TexSampler_";
const char* fpetexenvRGBScale_code = "_fpe4es_TexEnvRGBScale_";
const char* fpetexenvAlphaScale_code = "_fpe4es_TexEnvAlphaScale_";
const char* fpetexAdjust_code = "_fpe4es_TexAdjust_";
const char* fog_code = "_fpe4es_Fog.";
const char* vtx_progenv_noa = "_fpe4es_Vertex_ProgramEnv_";
const char* vtx_progenv_arr = "_fpe4es_Vertex_ProgramEnv[";
const char* vtx_progloc_noa = "_fpe4es_Vertex_ProgramLocal_";
const char* vtx_progloc_arr = "_fpe4es_Vertex_ProgramLocal[";
const char* frg_progenv_noa = "_fpe4es_Fragment_ProgramEnv_";
const char* frg_progenv_arr = "_fpe4es_Fragment_ProgramEnv[";
const char* frg_progloc_noa = "_fpe4es_Fragment_ProgramLocal_";
const char* frg_progloc_arr = "_fpe4es_Fragment_ProgramLocal[";
const char* samplers1d_noa = "_fpe4es_Sampler1D_";
const char* samplers2d_noa = "_fpe4es_Sampler2D_";
const char* samplers3d_noa = "_fpe4es_Sampler3D_";
const char* samplersCube_noa = "_fpe4es_SamplerCube_";
const char* blend_color_code = "_fpe4es_BlendColor";
int builtin_CheckUniform(program_t* glprogram, char* name, GLint id, int size) {
    if (strncmp(name, fpe4es_code, strlen(fpe4es_code)))
        return 0; // doesn't start with "_fpe4es_", no need to look further
