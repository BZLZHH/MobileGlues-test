...
void APIENTRY_FPE4ES fpe4es_glLineStipple(GLuint factor, GLushort pattern) {
    ...
    if(pattern!=glstate->linestipple.pattern || factor!=glstate->linestipple.factor || !glstate->linestipple.texture) {
        ...
        if (! glstate->linestipple.texture) {
            fpe4es_glGenTextures(1, &glstate->linestipple.texture);
            fpe4es_glBindTexture(GL_TEXTURE_2D, glstate->linestipple.texture);
            fpe4es_glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
            fpe4es_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
            fpe4es_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
