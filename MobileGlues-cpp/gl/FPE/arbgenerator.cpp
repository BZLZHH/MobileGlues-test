...
#define PUSH_SWIZZLE(s)                                                                                                \
    switch (s) {                                                                                                       \
    case _sInstruction_Vars::SWIZ_X:                                                                                   \
        APPEND_OUTPUT("x", 1);                                                                                         \
        break;                                                                                                         \
    case _sInstruction_Vars::SWIZ_Y:                                                                                   \
        APPEND_OUTPUT("y", 1);                                                                                         \
        break;                                                                                                         \
    case _sInstruction_Vars::SWIZ_Z:                                                                                   \
        APPEND_OUTPUT("z", 1);                                                                                         \
        break;                                                                                                         \
    case _sInstruction_Vars::SWIZ_W:                                                                                   \
        APPEND_OUTPUT("w", 1);                                                                                         \
        break;                                                                                                         \
    case _sInstruction_Vars::SWIZ_NONE:                                                                                \
        break;                                                                                                         \
    }
...
#define ASSERT_MASKDST(i)                                                                                              \
    ...
    for (int sw = 0; (sw < 3) && (SWIZ(i, sw + 1) != _sInstruction_Vars::SWIZ_NONE); ++sw) {                           \
...
#define ASSERT_VECTSRC(i)                                                                                              \
    ...
    if ((SWIZ(i, 1) != _sInstruction_Vars::SWIZ_NONE) && (SWIZ(i, 3) == _sInstruction_Vars::SWIZ_NONE)) {              \
        FAIL("Variable is not a valid vector source register");                                                        \
    }
#define ASSERT_SCALSRC(i)                                                                                              \
    ...
    if ((SWIZ(i, 0) == _sInstruction_Vars::SWIZ_NONE) || (SWIZ(i, 1) != _sInstruction_Vars::SWIZ_NONE)) {              \
        FAIL("Variable is not a valid vector source scalar");                                                          \
    }
#define INST_VECTOR                                                                                                    \
    ASSERT_COUNT(2)                                                                                                    \
...
#define INST_SAMPLE                                                                                                    \
    ...
    if ((instPtr->vars[3].var != curStatusPtr->tex1D) && (instPtr->vars[3].var != curStatusPtr->tex2D) &&              \
        (instPtr->vars[3].var != curStatusPtr->tex3D) &&                                                               \
        (instPtr->vars[3].var != curStatusPtr->texCUBE) /* && (instPtr->vars[3].var != curStatusPtr->texRECT) */) {    \
...
#define PUSH_DSTMASK(i, b)                                                                                             \
    if (((b == i) || (SWIZ(b, 0) == _sInstruction_Vars::SWIZ_NONE)) &&                                                 \
        (SWIZ(i, 0) != _sInstruction_Vars::SWIZ_NONE)) {                                                               \
        APPEND_OUTPUT(".", 1)                                                                                          \
        for (int sw = 0; (sw < 4) && (SWIZ(i, sw) != _sInstruction_Vars::SWIZ_NONE); ++sw) {                           \
            PUSH_SWIZZLE(SWIZ(i, sw))                                                                                  \
        }                                                                                                              \
    } else if ((b != i) && (SWIZ(b, 0) != _sInstruction_Vars::SWIZ_NONE) &&                                            \
               (SWIZ(i, 0) == _sInstruction_Vars::SWIZ_NONE)) {                                                        \
        APPEND_OUTPUT(".", 1) /* b is a vector */                                                                      \
        if (SWIZ(b, 1) == _sInstruction_Vars::SWIZ_NONE) {                                                             \
...
#define PUSH_DESTLEN(i)                                                                                                \
    for (dstSwizLen = 0; (dstSwizLen < 4) && (SWIZ(i, dstSwizLen) != _sInstruction_Vars::SWIZ_NONE); ++dstSwizLen)     \
        ;                                                                                                              \
    switch (dstSwizLen) {                                                                                              \
    case 1:                                                                                                            \
        APPEND_OUTPUT("(", 1)                                                                                          \
...
#define PUSH_VECTSRC(i)                                                                                                \
    PUSH_VARNAME(i)                                                                                                    \
    if (SWIZ(i, 0) != _sInstruction_Vars::SWIZ_NONE) {                                                                 \
        APPEND_OUTPUT(".", 1)                                                                                          \
        PUSH_SWIZZLE(SWIZ(i, 0))                                                                                       \
        if (SWIZ(i, 3) == _sInstruction_Vars::SWIZ_NONE) {                                                             \
            PUSH_SWIZZLE(SWIZ(i, 0))                                                                                   \
            PUSH_SWIZZLE(SWIZ(i, 0))                                                                                   \
            PUSH_SWIZZLE(SWIZ(i, 0))                                                                                   \
        } else {                                                                                                       \
...
#define FINISH_INST(dst)                                                                                               \
    ...
    case INST_ARL:
        ...
        if ((SWIZ(0, 0) != _sInstruction_Vars::SWIZ_X) || (SWIZ(0, 1) != _sInstruction_Vars::SWIZ_NONE)) {
