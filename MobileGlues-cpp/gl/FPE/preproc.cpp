...
eTokenType NextTokenComment(char** p, uToken* tok) {
    ...
    default:
        ...
        while (cnt != (MAXSTR - 1) && (nextc == '_' || (nextc >= '0' && nextc <= '9') ||
                                       (nextc >= 'A' && nextc <= 'Z') || (nextc >= 'a' && nextc <= 'z'))) {
            tok->str[cnt] = nextc;
            (*p)++;
            nextc = **p;
            ++cnt;
...
eTokenType NextToken(char** p, uToken* tok) {
    ...
    case '#':
        ...
        break;
    case '(':
        tok->type = TK_OPENBRACE;
        break;
    case ')':
        tok->type = TK_CLOSEBRACE;
        break;
    case '{':
        tok->type = TK_OPENCURLY;
        break;
    case '}':
        tok->type = TK_CLOSECURLY;
        break;
    case '[':
        tok->type = TK_OPENBRAKET;
        break;
    case ']':
        tok->type = TK_CLOSEBRAKET;
        break;
    case '&':
        tok->type = TK_AMP;
        break;
...
static int pop_if(stackif_t* st) {
    if (st->sz) return st->ifs[--st->sz];
    return -1;
}

static int top_if(stackif_t* st) {
    if (st->sz) return st->ifs[st->sz - 1];
    return -1;
}
static void not_top_if(stackif_t* st) {
    if (st->sz && st->ifs[st->sz - 1] != -1) st->ifs[st->sz - 1] = 1 - st->ifs[st->sz - 1];
}
...
char* preproc(const char* code, int keepcomments, int gl_es, extensions_t* exts, char** versionString) {
    ...
    while (tok.type != TK_NULL) {
        ...
        if (tok.type != TK_NULL) {
            ...
            // block comment...
            case 200:
                if (tok.type == TK_NEWLINE) newline = 1;
                ...
                if (tok.type != TK_SPACE)
                    if (tok.type == TK_TEXT) {
                        if (!strcmp(tok.str, "ifdef"))
                            ...
                            status = 810;
                            if (!*versionString) *versionString = (char*)calloc(1, 51);
                        } else
                            status = 399;
                    } else
                        status = 399; // meh?
            ...
            case 390:
                if (tok.type == TK_NEWLINE) {
                    ...
                    if (!strcmp(tok.str, "defined")) status = 710;
                }
                break;
            ...
            if (notok)
                notok = 0;
            else if (write && !oldp && nowrite_ifs != 1) {
    ...
    kh_foreach_value(alldefines, defname, free(defname);) kh_destroy(alldefine, alldefines);
    if (stackif.ifs) free(stackif.ifs);
