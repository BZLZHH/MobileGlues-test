...
eVariableType STR2VARTYPE(char* str) {
    return TST_STR_VAL_PREF(str, VARTYPE_, ADDRESS) TST_STR_VAL_PREF(str, VARTYPE_, ATTRIB)
        TST_STR_VAL_PREF(str, VARTYPE_, PARAM) TST_STR_VAL_PREF(str, VARTYPE_, TEMP)
            TST_STR_VAL_PREF(str, VARTYPE_, ALIAS) TST_STR_VAL_PREF(str, VARTYPE_, OUTPUT) VARTYPE_UNK;
}
eInstruction STR2INST(char* str, int* sat) {
    ...
    *sat = !strcmp(str + 3, "_SAT");
    if ((((str[0] < 'A') || (str[0] > 'Z')) && ((str[0] < '0') || (str[0] > '9'))) ||
        ...
        (((str[2] < 'A') || (str[2] > 'Z')) && ((str[2] < '0') || (str[2] > '9'))) || ((str[3] != '\0') && !*sat)) {
        return INST_UNK;
    }
...
void initStatus(sCurStatus* curStatus, const char* code) {
    ...
    curStatus->valueType = _sCurStatus::TYPE_NONE;
    ...
    curStatus->fogType = _sCurStatus::FOG_NONE;
...
void freeStatus(sCurStatus* curStatus) {
    ...
    } else if (curStatus->valueType == _sCurStatus::TYPE_ALIAS_DECL) {
        if (curStatus->curValue.string) {
            free(curStatus->curValue.string);
        }
    } else if (curStatus->valueType == _sCurStatus::TYPE_OPTION_DECL) {
...
int appendString(sCurStatus* curStatusPtr, const char* str, size_t strLen) {
    ...
    ARBCONV_DBG_HEAVY(ARBCONV_DBG(
        char* dup = malloc((strLen + 1) * sizeof(char)); memcpy(dup, str, strLen); dup[strLen] = '\0';
        printf("Appending '%s' to %p (%p + %ld = %p)\n", dup, curStatusPtr->outputEnd, curStatusPtr->outputString,
               curStatusPtr->outLen, curStatusPtr->outputString + curStatusPtr->outLen);
        free(dup); fflush(stdout);) if (curStatusPtr->outputEnd != curStatusPtr->outputString + curStatusPtr->outLen) {
