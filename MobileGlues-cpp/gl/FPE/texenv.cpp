...
void APIENTRY_FPE4ES fpe4es_glTexEnvf(GLenum target, GLenum pname, GLfloat param) {
    DBG(printf("glTexEnvf(%s, %s, 0x%04X(%s)), tmu=%d, pending=%d, compiling=%d\n", PrintEnum(target), PrintEnum(pname),
               (GLenum)param, PrintEnum((GLenum)param), glstate->texture.active, glstate->list.pending,
    ...
    case GL_POINT_SPRITE:
        if (pname == GL_COORD_REPLACE) {
            ...
            if (glstate->fpe_state) glstate->fpe_state->pointsprite_coord = p;
        } else {
    ...
    case GL_TEXTURE_ENV: {
        ...
        case GL_TEXTURE_ENV_MODE:
            ...
            } else if (param != GL_ADD && param != GL_MODULATE && param != GL_DECAL && param != GL_BLEND &&
                       param != GL_REPLACE && param != GL_COMBINE) {
                errorShim(GL_INVALID_ENUM);
                return;
            }
            FLUSH_BEGINEND;
        ...
        case GL_COMBINE_RGB:
            ...
            if ((param == GL_MODULATE_ADD_ATI || param == GL_MODULATE_SIGNED_ADD_ATI ||
                 param == GL_MODULATE_SUBTRACT_ATI)) {
                if (hardext.esversion == 1) {
                    errorShim(GL_INVALID_ENUM);
                    return;
                }
            } else if (param != GL_REPLACE && param != GL_MODULATE && param != GL_ADD && param != GL_ADD_SIGNED &&
                       param != GL_INTERPOLATE && param != GL_SUBTRACT && param != GL_DOT3_RGB &&
                       param != GL_DOT3_RGBA) {
                errorShim(GL_INVALID_ENUM);
                return;
            }
            ...
            if (glstate->fpe_state) {
                ...
                case GL_MODULATE:
                    state = FPE_CR_MODULATE;
                    break;
                case GL_ADD:
                    state = FPE_CR_ADD;
                    break;
                case GL_ADD_SIGNED:
                    state = FPE_CR_ADD_SIGNED;
                    break;
                case GL_INTERPOLATE:
                    state = FPE_CR_INTERPOLATE;
                    break;
                case GL_SUBTRACT:
                    state = FPE_CR_SUBTRACT;
                    break;
                case GL_DOT3_RGB:
                    state = FPE_CR_DOT3_RGB;
                    break;
                case GL_DOT3_RGBA:
                    state = FPE_CR_DOT3_RGBA;
                    break;
                case GL_MODULATE_ADD_ATI:
                    state = FPE_CR_MOD_ADD;
                    break;
                case GL_MODULATE_SIGNED_ADD_ATI:
                    state = FPE_CR_MOD_ADD_SIGNED;
                    break;
                case GL_MODULATE_SUBTRACT_ATI:
                    state = FPE_CR_MOD_SUB;
                    break;
                }
        ...
        case GL_COMBINE_ALPHA:
            ...
            break;
        case GL_SRC0_RGB:
            if (t->src0_rgb == param) return;
            if ((param == GL_ZERO || param == GL_ONE || param == GL_SECONDARY_COLOR_ATIX ||
                 param == GL_TEXTURE_OUTPUT_RGB_ATIX)) {
                if (hardext.esversion == 1) {
                    errorShim(GL_INVALID_ENUM);
                    return;
                }
            ...
            if (glstate->fpe_state) {
                ...
                if (t->src0_rgb >= GL_TEXTURE0 && t->src0_rgb < GL_TEXTURE0 + MAX_TEX) {
                    ...
                    switch (t->src0_rgb) {
                    case GL_CONSTANT:
                        state = FPE_SRC_CONSTANT;
                        break;
                    case GL_PRIMARY_COLOR:
                        state = FPE_SRC_PRIMARY_COLOR;
                        break;
                    case GL_PREVIOUS:
                        state = FPE_SRC_PREVIOUS;
                        break;
                    }
            ...
            if (glstate->fpe_state) {
                ...
                if (t->src2_rgb >= GL_TEXTURE0 && t->src2_rgb < GL_TEXTURE0 + MAX_TEX) {
                    ...
                    case GL_ZERO:
                        state = FPE_SRC_ZERO;
                        break;
                    case GL_SECONDARY_COLOR_ATIX:
                        state = FPE_SRC_SECONDARY_COLOR;
                        break;
        ...
        case GL_SRC3_RGB:
            ...
            break;
        case GL_SRC0_ALPHA:
            if (t->src0_alpha == param) return;
            if ((param == GL_ZERO || param == GL_ONE || param == GL_TEXTURE_OUTPUT_ALPHA_ATIX)) {
                if (hardext.esversion == 1) {
                    errorShim(GL_INVALID_ENUM);
                    return;
                }
        ...
        case GL_OPERAND0_RGB:
            if (t->op0_rgb == param) return;
            if (param != GL_SRC_COLOR && param != GL_ONE_MINUS_SRC_COLOR && param != GL_SRC_ALPHA &&
                param != GL_ONE_MINUS_SRC_ALPHA) {
                errorShim(GL_INVALID_ENUM);
                return;
            }
            FLUSH_BEGINEND;
            ...
            if (glstate->fpe_state) {
                ...
                switch (t->op0_rgb) {
                case GL_SRC_COLOR:
                    state = FPE_OP_SRCCOLOR;
                    break;
                case GL_ONE_MINUS_SRC_COLOR:
                    state = FPE_OP_MINUSCOLOR;
                    break;
                case GL_ONE_MINUS_SRC_ALPHA:
                    state = FPE_OP_MINUSALPHA;
                    break;
                }
...
void APIENTRY_FPE4ES fpe4es_glTexEnvi(GLenum target, GLenum pname, GLint param) {
    DBG(printf("glTexEnvi(...)->");)
    fpe4es_glTexEnvf(target, pname, param);
}

void APIENTRY_FPE4ES fpe4es_glTexEnvfv(GLenum target, GLenum pname, const GLfloat* param) {
...
void APIENTRY_FPE4ES fpe4es_glTexEnviv(GLenum target, GLenum pname, const GLint* param) {
    ...
    if (target == GL_TEXTURE_ENV && pname == GL_TEXTURE_ENV_COLOR) {
        ...
        fpe4es_glTexEnvfv(target, pname, p);
    } else
        fpe4es_glTexEnvf(target, pname, *param);
}
void APIENTRY_FPE4ES fpe4es_glGetTexEnvfv(GLenum target, GLenum pname, GLfloat* params) {
    // FLUSH_BEGINEND;
...
void APIENTRY_FPE4ES fpe4es_glGetTexEnviv(GLenum target, GLenum pname, GLint* params) {
