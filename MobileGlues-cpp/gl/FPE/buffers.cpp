...
int buffer_target(GLenum target) {
    if (target == GL_ARRAY_BUFFER) return 1;
    if (target == GL_ELEMENT_ARRAY_BUFFER) return 1;
    if (target == GL_PIXEL_PACK_BUFFER) return 1;
    if (target == GL_PIXEL_UNPACK_BUFFER) return 1;
    return 0;
}
...
void APIENTRY_FPE4ES fpe4es_glGenBuffers(GLsizei n, GLuint* buffers) {
    ...
    for (int i = 0; i < n; i++) { // create buffer, and check uniqueness...
        ...
        glbuffer_t* buff = kh_value(list, k) = (glbuffer_t*)malloc(sizeof(glbuffer_t));
        buff->buffer = b;
...
void APIENTRY_FPE4ES fpe4es_glBindBuffer(GLenum target, GLuint buffer) {
    ...
    if (buffer == 0) {
        ...
        if (k == kh_end(list)) {
            ...
            buff = kh_value(list, k) = (glbuffer_t*)malloc(sizeof(glbuffer_t));
            buff->buffer = buffer;
            buff->type = target;
            buff->data = NULL;
            buff->usage = GL_STATIC_DRAW;
...
// void* APIENTRY_FPE4ES fpe4es_glMapBufferRange(GLenum target, GLintptr offset, GLsizeiptr length, GLbitfield access)
// {
...
//     }
//
//     if(buff->real_buffer && (buff->type==GL_ARRAY_BUFFER || buff->type==GL_ELEMENT_ARRAY_BUFFER) &&
...
void realize_bufferIndex() {
    if (glstate->bind_buffer.index != glstate->bind_buffer.want_index) {
        ...
        GLES.glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, glstate->bind_buffer.index);
...
void deleteSingleBuffer(GLuint buffer) {
    ...
    else if (glstate->bind_buffer.want_index == buffer)
        glstate->bind_buffer.want_index = 0;
    else if (glstate->bind_buffer.array == buffer)
