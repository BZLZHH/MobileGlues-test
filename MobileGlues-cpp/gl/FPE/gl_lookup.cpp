...
#define STUB_FCT fpe4es_Stub
#include "gl_lookup.h"

#if (!defined(_WIN32) || defined(_WIN64)) && !defined(__MINGW32__)
void fpe4es_Stub(void* x, ...) {
    return;
}
#else
// TODO: if need use STUB with different argnum, the macro must be redesigned
void APIENTRY_FPE4ES fpe4es_Stub(void* x) {}
#endif

void* APIENTRY_FPE4ES fpe4es_GetProcAddress(const char* name) {
...
#undef THUNK
    ...
    DBG(printf("NULL\n");)
    if (!globals4es.silentstub) LOGD("FPE4ES GetProcAddress: %s not found.\n", name);
    return NULL;
}
