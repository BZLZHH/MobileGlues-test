...
#define ShadAppend(S) shad = fpe4es_append(shad, &shad_cap, S)
...
const char* fpe4es_alphaRefSource = "uniform float _fpe4es_AlphaRef;\n";
...
const char* fpe_texenvSrc(int src, int tmu, int twosided) {
    ...
    switch(src) {
        ...
        case FPE_SRC_CONSTANT:
            sprintf(buff, "_fpe4es_TextureEnvColor_%d", tmu);
            break;
...
const char* const* fpe_VertexShader(shaderconv_need_t* need, fpe_state_t *state) {
    ...
    if(planes) {
        for (int i=0; i<hardext.maxplanes; i++) {
            if((planes>>i)&1) {
                sprintf(buff, "uniform highp vec4 _fpe4es_ClipPlane_%d;\n", i);
                ShadAppend(buff);
                ++headers;
    ...
    if(lighting) {
        sprintf(buff, 
            "struct _fpe4es_FPELightSourceParameters1\n"
            "{\n"
            "%s"
            "   highp vec4 specular;\n"
            "   highp vec4 position;\n"
        ...
        headers += fpe4es_countline(buff);
        sprintf(buff, 
            "struct _fpe4es_FPELightSourceParameters0\n"
            "{\n"
            "%s"
            "   highp vec4 specular;\n"
            "   highp vec4 position;\n"
        ...
        if(!(cm_front_nullexp && color_material)) {
            ShadAppend("uniform highp float _fpe4es_FrontMaterial_shininess;\n");
            headers++;
        }
        if(twosided && !(cm_back_nullexp && color_material)) {
            ShadAppend("uniform highp float _fpe4es_BackMaterial_shininess;\n");
            headers++;
        }
        if(!(color_material && (state->cm_front_mode==FPE_CM_DIFFUSE || state->cm_front_mode==FPE_CM_AMBIENTDIFFUSE))) {
            ShadAppend("uniform highp float _fpe4es_FrontMaterial_alpha;\n");
            headers++;
            if(twosided) {
                ShadAppend("uniform highp float _fpe4es_BackMaterial_alpha;\n");
                headers++;
            }
        }
        for(int i=0; i<hardext.maxlights; i++) {
            if(state->light&(1<<i)) {
                sprintf(buff, "uniform _fpe4es_FPELightSourceParameters%d _fpe4es_LightSource_%d;\n", (state->light_direction>>i&1)?1:0, i);
                ShadAppend(buff);
                headers++;

                sprintf(buff, "uniform _fpe4es_LightProducts _fpe4es_FrontLightProduct_%d;\n", i);
                ShadAppend(buff);
                headers++;

                if(twosided) {
                    sprintf(buff, "uniform _fpe4es_LightProducts _fpe4es_BackLightProduct_%d;\n", i);
                    ShadAppend(buff);
                    headers++;
                }
            }
    ...
    for (int i=0; i<hardext.maxtex; i++) {
        ...
        if(t) {
            sprintf(buff, "varying %s _fpe4es_TexCoord_%d;\n", texvecsize[t-1], i);
            ShadAppend(buff);
            headers++;
            if(state->texture[i].texmat) {
                sprintf(buff, "uniform highp mat4 _fpe4es_TextureMatrix_%d;\n", i);
                ShadAppend(buff);
                headers++;
            }
        }
    ...
    ShadAppend("\nvoid main() {\n");
    int need_normal = 0;
    int normal_line = fpe4es_countline(shad) - headers;
    if(planes) {
        for (int i=0; i<hardext.maxplanes; i++) {
            if((planes>>i)&1) {
                sprintf(buff, "clippedvertex_%d = dot(vertex, _fpe4es_ClipPlane_%d);\n", i, i);
                ShadAppend(buff);
            }
        }
        if(!need_vertex)
    ...
    if(!lighting) {
        ...
        sprintf(fm_diffuse, "%s", (color_material && (state->cm_front_mode==FPE_CM_DIFFUSE || state->cm_front_mode==FPE_CM_AMBIENTDIFFUSE))?"gl_Color.xyz * _fpe4es_LightSource_":"_fpe4es_FrontLightProduct_");
        sprintf(fm_specular, "%s", (color_material && state->cm_front_mode==FPE_CM_SPECULAR)?"gl_Color.xyz * _fpe4es_LightSource_":"_fpe4es_FrontLightProduct_");
        if(twosided) {
            ...
            sprintf(bm_diffuse, "%s", (color_material && (state->cm_back_mode==FPE_CM_DIFFUSE || state->cm_back_mode==FPE_CM_AMBIENTDIFFUSE))?"gl_Color.xyz * _fpe4es_LightSource_":"_fpe4es_BackLightProduct_");
            sprintf(bm_specular, "%s", (color_material && state->cm_back_mode==FPE_CM_SPECULAR)?"gl_Color.xyz * _fpe4es_LightSource_":"_fpe4es_BackLightProduct_");
        }
        ...
        for(int i=0; i<hardext.maxlights; i++) {
            if(state->light&(1<<i)) {
                ...
                if((state->light_direction>>i&1)==0) { // flag is 1 if light is has w!=0
                    ShadAppend("att = 1.0;\n");
                    sprintf(buff, "VP = normalize(_fpe4es_LightSource_%d.position.xyz);\n", i);
                    ShadAppend(buff);
                } else {
                    sprintf(buff, "VP = _fpe4es_LightSource_%d.position.xyz - vertex.xyz;\n", i);
                    ShadAppend(buff);
                    ShadAppend("lVP = length(VP);\n");
                    sprintf(buff, "att = 1.0/(_fpe4es_LightSource_%d.constantAttenuation + lVP*(_fpe4es_LightSource_%d.linearAttenuation + _fpe4es_LightSource_%d.quadraticAttenuation * lVP));\n", i, i, i);
                    ShadAppend(buff);
                ...
                if((state->light_cutoff180>>i&1)==0) {
                    ...
                    /*if((state->light_direction>>i&1)==0) {
                        sprintf(buff, "spot = max(dot(-normalize(vertex.xyz), _fpe4es_LightSource_%d.spotDirection), 0.);\n", i);
                        if(!need_vertex) need_vertex=1;
                    } else*/ {
                        sprintf(buff, "spot = max(dot(-VP, _fpe4es_LightSource_%d.spotDirection), 0.);\n", i);
                    }
                    ShadAppend(buff);
                    sprintf(buff, "if(spot<_fpe4es_LightSource_%d.spotCosCutoff) spot=0.0; else spot=pow(spot, _fpe4es_LightSource_%d.spotExponent);\n", i, i);
                    ShadAppend(buff);
                    ShadAppend("att *= spot;\n");
                }
                if(color_material && (state->cm_front_mode==FPE_CM_AMBIENT || state->cm_front_mode==FPE_CM_AMBIENTDIFFUSE)) {
                    sprintf(buff, "aa = %s.xyz * _fpe4es_LightSource_%d.ambient.xyz;\n", fm_ambient, i);
                    ShadAppend(buff);
                } else {
                    sprintf(buff, "aa = _fpe4es_FrontLightProduct_%d.ambient.xyz;\n", i);
                    ShadAppend(buff);
                    need_lightproduct[0][i] = 1;
                }
                if(twosided) {
                    if(color_material && (state->cm_back_mode==FPE_CM_AMBIENT || state->cm_back_mode==FPE_CM_AMBIENTDIFFUSE)) {
                        sprintf(buff, "back_aa = %s.xyz * _fpe4es_LightSource_%d.ambient.xyz;\n", bm_ambient, i);
                        ShadAppend(buff);
                    } else {
                        sprintf(buff, "back_aa = _fpe4es_BackLightProduct_%d.ambient.xyz;\n", i);
                        ShadAppend(buff);
        ...
        sprintf(buff, "Color.a = %s;\n", (color_material && (state->cm_front_mode==FPE_CM_DIFFUSE || state->cm_front_mode==FPE_CM_AMBIENTDIFFUSE))?"gl_Color.a":"_fpe4es_FrontMaterial_alpha");
        ShadAppend(buff);
        ShadAppend("Color.rgb = clamp(Color.rgb, 0., 1.);\n");
        if(twosided) {
            sprintf(buff, "BackColor.a = %s;\n", (color_material && (state->cm_back_mode==FPE_CM_DIFFUSE || state->cm_back_mode==FPE_CM_AMBIENTDIFFUSE))?"gl_Color.a":"_fpe4es_BackMaterial_alpha");
    ...
    for (int i=0; i<hardext.maxtex; i++) {
        ...
        if(t) {
            ...
            if (tg[0] || tg[1] || tg[2] || tg[3]) {
                ...
                for (int j=0; j<ntc; j++) {
                    if(tg[j]==FPE_TG_NORMALMAP) {
                        ...
                        sprintf(buff, "tmp_tcoor.%c=dot(gl_Vertex, _fpe4es_ObjectPlane%c_%d);\n", texcoordxy[j], texcoordNAME[j], i);
                        need_objplane[i][j] = 1;
                    } else if(tg[j]==FPE_TG_EYELINEAR) {
                        sprintf(buff, "tmp_tcoor.%c=dot(vertex, _fpe4es_EyePlane%c_%d);\n", texcoordxy[j], texcoordNAME[j], i);
                        need_eyeplane[i][j] = 1;
                        if(!need_vertex) need_vertex=1;
            ...
            if(mat) {
                text_tmp = tmp_tex;
                sprintf(buff, "%s = (_fpe4es_TextureMatrix_%d * %s);\n", text_tmp, i, texcoord);
                ShadAppend(buff);
            }
            if(t==FPE_TEX_STRM) {
                sprintf(buff, "_fpe4es_TexCoord_%d = %s.%s / %s.q;\n", i, text_tmp, texxyzsize[t-1], text_tmp);
            } else {
                sprintf(buff, "_fpe4es_TexCoord_%d = %s.%s;\n", i, text_tmp, texxyzsize[t-1]);
            }
            ShadAppend(buff);
            if(adjust) {
                need_adjust[i] = 1;
                sprintf(buff, "_fpe4es_TexCoord_%d.xy *= _fpe4es_TexAdjust_%d;\n", i, i);    // to avoid error on Cube map... but will that work anyway?
                ShadAppend(buff);
            }
        }
    }
    ...
    if(need_vertex) {
        ...
        shad = fpe4es_inplace_insert(fpe4es_getline(shad, normal_line + headers), buff, shad, &shad_cap);
        normal_line += fpe4es_countline(buff);
    }
    if(need_normal) {
#if 0
        if(state->rescaling)
    ...
    for (int i=0; i<MAX_TEX; i++) {
        ...
        for (int j=0; j<4; j++) {
            if(need_objplane[i][j]) {
                sprintf(tmp, "uniform vec4 _fpe4es_ObjectPlane%c_%d;\n", texcoordNAME[j], i);
                strcat(buff, tmp);
            }
            if(need_eyeplane[i][j]) {
                sprintf(tmp, "uniform vec4 _fpe4es_EyePlane%c_%d;\n", texcoordNAME[j], i);
                strcat(buff, tmp);
            }
        }
        if(need_adjust[i]) {
            sprintf(tmp, "uniform vec2 _fpe4es_TexAdjust_%d;\n", i);
            strcat(buff, tmp);
        }
    }
    if(buff[0]!='\0') {
        shad = fpe4es_inplace_insert(fpe4es_getline(shad, headers), buff, shad, &shad_cap);
        headers += fpe4es_countline(buff);
    }
    if(fog) {
        if(comments) {
...
const char* const* fpe_FragmentShader(shaderconv_need_t* need, fpe_state_t *state) {
    ...
    for (int i=0; i<hardext.maxtex; i++) {
        ...
        if(t) {
            sprintf(buff, "varying %s _fpe4es_TexCoord_%d;\n", texvecsize[t-1], i);
            ShadAppend(buff);
            sprintf(buff, "uniform %s _fpe4es_TexSampler_%d;\n", texsampler[t-1], i);
            ShadAppend(buff);
            headers++;
            ...
            if (texenv>=FPE_COMBINE) {
                ...
                if(state->texenv[i].texrgbscale) {
                    sprintf(buff, "uniform float _fpe4es_TexEnvRGBScale_%d;\n", i);
                    ShadAppend(buff);
                    headers++;
                }
                if(state->texenv[i].texalphascale) {
                    sprintf(buff, "uniform float _fpe4es_TexEnvAlphaScale_%d;\n", i);
                    ShadAppend(buff);
                    headers++;
                }
            }
    ...
    if(alpha_test && alpha_func>FPE_NEVER) {
        ShadAppend(fpe4es_alphaRefSource);
        headers++;
    } 
            ...
            if(t) {
                if(point && pointsprite && pointsprite_coord) {
                    if(pointsprite_upper)
                        sprintf(buff, "vec4 texColor%d = %s(_fpe4es_TexSampler_%d, vec2(gl_PointCoord.x, 1.-gl_PointCoord.y));\n", i, texnoproj[t-1], i);
                    else
                        sprintf(buff, "vec4 texColor%d = %s(_fpe4es_TexSampler_%d, gl_PointCoord);\n", i, texnoproj[t-1], i);
                } else
                    sprintf(buff, "vec4 texColor%d = %s(_fpe4es_TexSampler_%d, _fpe4es_TexCoord_%d);\n", i, texname[t-1], i, i);
                ShadAppend(buff);
            }
        }
            ...
            if(t) {
                ...
                switch (texenv) {
                    ...
                    case FPE_BLEND:
                        ...
                        sprintf(buff, "uniform highp vec4 _fpe4es_TextureEnvColor_%d;\n", i);
                        shad = fpe4es_inplace_insert(fpe4es_getline(shad, headers), buff, shad, &shad_cap);
                        headers+=fpe4es_countline(buff);
                        needclamp=0;
                        if(texformat!=FPE_TEX_ALPHA) {
                            sprintf(buff, "fColor.rgb = mix(fColor.rgb, _fpe4es_TextureEnvColor_%d.rgb, texColor%d.rgb);\n", i, i);
                            ShadAppend(buff);
                        }
                        switch(texformat) {
                            case FPE_TEX_LUM:
                            ...
                            case FPE_TEX_DEPTH:
                                sprintf(buff, "fColor.a = mix(fColor.a, _fpe4es_TextureEnvColor_%d.a, texColor%d.a);\n", i, i);
                                ShadAppend(buff);
                                break;
                            default:
                                sprintf(buff, "fColor.a *= texColor%d.a;\n", i);
                    ...
                    case FPE_COMBINE4:
                        {
                            ...
                            if((state->texenv[i].texrgbscale) && (state->texenv[i].texalphascale)) {
                                sprintf(buff, "fColor *= _fpe4es_TexEnvRGBScale_%d;\n", i);
                                ShadAppend(buff);
                            } else {
                                if(state->texenv[i].texrgbscale) {
                                    sprintf(buff, "fColor.rgb *= _fpe4es_TexEnvRGBScale_%d;\n", i);
                                    ShadAppend(buff);
                                }
                                if(state->texenv[i].texalphascale) {
                                    sprintf(buff, "fColor.a *= _fpe4es_TexEnvAlphaScale_%d;\n", i);
                                    ShadAppend(buff);
                                }
                            }
                        }
    ...
    if(alpha_test) {
        ...
        if(alpha_func==FPE_ALWAYS) {
            ...
            sprintf(buff, "if (floor(fColor.a*255.) %s _fpe4es_AlphaRef) discard;\n", alpha_test_op[alpha_func-FPE_LESS]);
            ShadAppend(buff);
        }
    }
            ...
            if(blendrgb==blendalpha) {
                ...
                switch(blendrgb) {
                    ...
                    case FPE_BLEND_CONSTANT_COLOR:
                        need_vec4 = 1;
                        sprintf(buff, " %s = _fpe4es_BlendColor;\n", blend);
                        break;
                    case FPE_BLEND_ONE_MINUS_CONSTANT_COLOR:
                        need_vec4 = 1;
                        sprintf(buff, " %s = vec4(1.0)-_fpe4es_BlendColor;\n", blend);
                        break;
                    case FPE_BLEND_CONSTANT_ALPHA:
                        sprintf(buff, " %s = _fpe4es_BlendColor.a;\n", blend);
                        break;
                    case FPE_BLEND_ONE_MINUS_CONSTANT_ALPHA:
                        sprintf(buff, " %s = 1.0 - _fpe4es_BlendColor.a;\n", blend);
                        break;
                ...
                switch(blendrgb) {
                    ...
                    case FPE_BLEND_ONE_MINUS_DST_ALPHA:
                        ...
                        break;
                    case FPE_BLEND_CONSTANT_COLOR:
                        sprintf(buff, " %s.rgb = _fpe4es_BlendColor.rgb;\n", blend);
                        break;
                    case FPE_BLEND_ONE_MINUS_CONSTANT_COLOR:
                        sprintf(buff, " %s.rgb = vec3(1.0)-_fpe4es_BlendColor.rgb;\n", blend);
                        break;
                    case FPE_BLEND_CONSTANT_ALPHA:
                        sprintf(buff, " %s.rgb = vec3(_fpe4es_BlendColor.a);\n", blend);
                        break;
                    case FPE_BLEND_ONE_MINUS_CONSTANT_ALPHA:
                        sprintf(buff, " %s.rgb = vec3(1.0 - _fpe4es_BlendColor.a);\n", blend);
                        break;
                    case FPE_BLEND_SRC_ALPHA_SATURATE:
                ...
                switch(blendalpha) {
                    ...
                    case FPE_BLEND_ONE_MINUS_DST_ALPHA:
                        ...
                        break;
                    case FPE_BLEND_CONSTANT_COLOR:
                        sprintf(buff, " %s.a = _fpe4es_BlendColor.a;\n", blend);
                        break;
                    case FPE_BLEND_ONE_MINUS_CONSTANT_COLOR:
                        sprintf(buff, " %s.a = 1.0-_fpe4es_BlendColor.a;\n", blend);
                        break;
                    case FPE_BLEND_CONSTANT_ALPHA:
                        sprintf(buff, " %s.a = _fpe4es_BlendColor.a;\n", blend);
                        break;
...
const char* const* fpe_CustomVertexShader(const char* initial, fpe_state_t* state, int default_fragment)
{
    ...
    if(!shad_cap) shad_cap = 1024;
    if(!shad) shad = (char*)malloc(shad_cap);
    int headline = fpe4es_getline_for(initial, "main");
    if(headline) --headline;

    strcpy(shad, "");
    ShadAppend(initial);

    int color = default_fragment?(strstr(initial, "_fpe4es_Color")?0:1):0;   // need to add a simple color variant?
if(default_fragment) printf("fpe_CustomVertexShader(%p, %p, %d)\n%s\ncolor=%d\n", initial, state, default_fragment, initial, color);
    ...
    if(color) {
        sprintf(buff, "attribute highp vec4 _fpe4es_Color;\n");
        ShadAppend(buff);
        ++headline;
    ...
    if(planes || color) {
        // wrap real main...
        shad = fpe4es_inplace_replace(shad, &shad_cap, "main", "_fpe4es_main");
    }

    // let's start
    if(strstr(shad, "_fpe4es_main")) {
        ShadAppend("\nvoid main() {\n");
        if(color) {
            sprintf(buff, "Color = _fpe4es_Color;\n");
        }
        ShadAppend("_fpe4es_main();\n");
        if(planes) {
            int clipvertex = 0;
            if(strstr(shad, "fpe4es_ClipVertex"))
                clipvertex = 1;
            for (int i=0; i<hardext.maxplanes; i++) {
                if((planes>>i)&1) {
                    sprintf(buff, "clippedvertex_%d = dot(%s, _fpe4es_ClipPlane_%d);\n", i, clipvertex?"fpe4es_ClipVertex":"gl_ModelViewMatrix * gl_Vertex", i);
                    ShadAppend(buff);
                }
            }
        }
...
const char* const* fpe_CustomFragmentShader(const char* initial, fpe_state_t* state)
{
    ...
    if(shaderblend && (
     ...
     || (state->blenddstalpha>=FPE_BLEND_CONSTANT_COLOR && state->blenddstalpha<=FPE_BLEND_ONE_MINUS_CONSTANT_ALPHA)
        ...
        sprintf(buff, "uniform highp vec4 _fpe4es_BlendColor;\n");
        ShadAppend(buff);
    }
    ...
    if(alpha_test || planes || shaderblend) {
        ...
        shad = fpe4es_inplace_replace(shad, &shad_cap, "main", "_fpe4es_main");
        if(is_fragcolor) {
            int l_main = fpe4es_getline_for(shad, fpe4es_prev_str(shad, strstr(shad, "_fpe4es_main"))) - 1;
            shad = fpe4es_inplace_insert(fpe4es_getline(shad, l_main), "highp vec4 _fpe4es_FragColor;\n", shad, &shad_cap);
            shad = fpe4es_inplace_replace(shad, &shad_cap, "gl_FragColor", "_fpe4es_FragColor");
        }
    }
    if(strstr(shad, "_fpe4es_main")) {
        ShadAppend("void main() {\n");
        ...
        if(alpha_test) {
            if(alpha_test && alpha_func>FPE_NEVER) {
                shad = fpe4es_inplace_insert(fpe4es_getline(shad, headline), fpe4es_alphaRefSource, shad, &shad_cap);
                headline+=fpe4es_countline(fpe4es_alphaRefSource);
            } 
            if(comments) {
                sprintf(buff, "// Alpha Test, fct=%X\n", alpha_func);
                ShadAppend(buff);
            ...
            if(alpha_func==FPE_ALWAYS) {
                ...
                // but need to negate the operator
                const char* alpha_test_op[] = {">=","!=",">","<=","==","<"}; 
                sprintf(buff, " if (floor(%s.a*255.) %s _fpe4es_AlphaRef) discard;\n", is_fragcolor?"_fpe4es_FragColor":"gl_FragData[0]", alpha_test_op[alpha_func-FPE_LESS]);
                ShadAppend(buff);
            }
        }
        ...
        //*** Blend in Shader
        if(shaderblend) {
            const char* frgcolor = is_fragcolor?"_fpe4es_FragColor":"gl_FragData[0]";
            const char* dstcolor = "gl_LastFragColorARM";
        ...
        if((alpha_test || planes || shaderblend) && is_fragcolor)
            ShadAppend("gl_FragColor = _fpe4es_FragColor;\n");
...
#ifdef FPE4ES_COMPILE_FOR_USE_IN_SHARED_LIB
