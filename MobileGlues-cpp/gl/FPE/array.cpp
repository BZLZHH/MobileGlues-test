...
GLvoid* copy_gl_array(const GLvoid* src, GLenum from, GLsizei width, GLsizei stride, GLenum to, GLsizei to_width,
                      GLsizei skip, GLsizei count, void* dst) {
    if (!src || !count) return NULL;

    if (!stride) stride = width * gl_sizeof(from);
    ...
    if (from == to && to_width >= width) {
        GL_TYPE_SWITCH(
            ...
            default : printf(unknown_str, from);
            return NULL;)
    } else {
        GL_TYPE_SWITCH(
            out, dst, to,
            for (int i = skip; i < count; i++) {
                GL_TYPE_SWITCH(
                    input, in, from,
                    for (int j = 0; j < width; j++) { out[j] = input[j]; } for (int j = width; j < to_width;
                                                                                j++) { out[j] = 0; } out += to_width;
                    in += stride;, default : printf(unknown_str, from); return NULL;)
            },
            default : printf(unknown_str, to);
            return NULL;)
    }
...
GLvoid* copy_gl_array_texcoord(const GLvoid* src, GLenum from, GLsizei width, GLsizei stride, GLsizei to_width,
                               GLsizei skip, GLsizei count, void* dst) {
    if (!src || !count) return NULL;
    ...
    if (!dst) dst = malloc((count - skip) * to_width * gl_sizeof(GL_FLOAT));
    GLsizei from_size = gl_sizeof(from) * width;
    ...
    if (from == GL_FLOAT && to_width >= width) {
        ...
        for (int i = skip; i < count; i++) {
            GL_TYPE_SWITCH(
                input, in, from,
                for (int j = 0; j < width; j++) { out[j] = input[j]; } for (int j = width; j < to_width; j++) {
                    if (j == 3)
                        out[j] = 1.0f;
                    else
                        out[j] = 0.0f;
...
GLvoid* copy_gl_array_quickconvert(const GLvoid* src, GLenum from, GLsizei stride, GLsizei skip, GLsizei count,
                                   void* dest) {

    if (!stride) stride = 4 * gl_sizeof(from);
    ...
    GLfloat* out = (GLfloat*)dst;
    GL_TYPE_SWITCH2(
        input, in, from, const GLfloat maxf = 1.0f / gl_max_value(from);
        for (int i = skip; i < count; i++), for (j = 0; j < 4; j++) { out[j] = ((GLfloat)input[j]) * maxf; } out += 4;
        in += stride;, default : printf(unknown_str, from); return NULL;)
    return dst;
}

GLvoid* copy_gl_array_convert(const GLvoid* src, GLenum from, GLsizei width, GLsizei stride, GLenum to,
                              GLsizei to_width, GLsizei skip, GLsizei count, GLvoid* filler, void* dst) {
    if (!src || !count) return NULL;
    ...
    if (from == to && to_width >= width) {
        ...
        GL_TYPE_SWITCH_MAX(
            out, dst, to,
            GL_TYPE_SWITCH2(
                input, in, from, const GLuint maxf = gl_max_value(from);
                for (int i = skip; i < count; i++),
                for (j = 0; j < width; j++) { out[j] = input[j] * maxv / maxf; } for (; j < to_width - 1; j++) {
                    out[j] = 0;
                } for (; j < to_width; j++) { memcpy(&out[j], filler, gl_sizeof(to)); } out += to_width;
                in += stride;, default : printf(unknown_str, from); return NULL;),
            default : printf(unknown_str, to);
            return NULL;)
    }
...
GLvoid* copy_gl_pointer(vertexattrib_t* ptr, GLsizei width, GLsizei skip, GLsizei count) {
    float filler = 0.0f;
    return copy_gl_array_convert((const GLvoid*)((uintptr_t)ptr->pointer + (uintptr_t)ptr->real_pointer), ptr->type,
                                 ptr->size, ptr->stride, GL_FLOAT, width, skip, count, &filler, NULL);
}
...
GLvoid* copy_gl_pointer_bytecolor(vertexattrib_t* ptr, GLsizei width, GLsizei skip, GLsizei count) {
    GLubyte filler = 255;
    return copy_gl_array_convert((const GLvoid*)((uintptr_t)ptr->pointer + (uintptr_t)ptr->real_pointer), ptr->type,
                                 ptr->size, ptr->stride, GL_UNSIGNED_BYTE, width, skip, count, &filler, NULL);
}

GLvoid* copy_gl_pointer_raw(vertexattrib_t* ptr, GLsizei width, GLsizei skip, GLsizei count) {
    return copy_gl_array((const GLvoid*)((uintptr_t)ptr->pointer + (uintptr_t)ptr->real_pointer), ptr->type, ptr->size,
                         ptr->stride, GL_FLOAT, width, skip, count, NULL);
}

GLvoid* copy_gl_pointer_tex(vertexattrib_t* ptr, GLsizei width, GLsizei skip, GLsizei count) {
    // float filler = 1.0f;
    return copy_gl_array_texcoord((const GLvoid*)((uintptr_t)ptr->pointer + (uintptr_t)ptr->real_pointer), ptr->type,
                                  ptr->size, ptr->stride,
...
void copy_gl_pointer_noalloc(void* dest, vertexattrib_t* ptr, GLsizei width, GLsizei skip, GLsizei count) {
    float filler = 0.0f;
    copy_gl_array_convert((const GLvoid*)((uintptr_t)ptr->pointer + (uintptr_t)ptr->real_pointer), ptr->type, ptr->size,
                          ptr->stride, GL_FLOAT, width, skip, count, &filler, dest);
}
...
void copy_gl_pointer_bytecolor_noalloc(void* dest, vertexattrib_t* ptr, GLsizei width, GLsizei skip, GLsizei count) {
    GLubyte filler = 255;
    copy_gl_array_convert((const GLvoid*)((uintptr_t)ptr->pointer + (uintptr_t)ptr->real_pointer), ptr->type, ptr->size,
                          ptr->stride, GL_UNSIGNED_BYTE, width, skip, count, &filler, dest);
}
void copy_gl_pointer_raw_noalloc(void* dest, vertexattrib_t* ptr, GLsizei width, GLsizei skip, GLsizei count) {
    copy_gl_array((const GLvoid*)((uintptr_t)ptr->pointer + (uintptr_t)ptr->real_pointer), ptr->type, ptr->size,
                  ptr->stride, GL_FLOAT, width, skip, count, dest);
}
void copy_gl_pointer_tex_noalloc(void* dest, vertexattrib_t* ptr, GLsizei width, GLsizei skip, GLsizei count) {
    // float filler = 1.0f;
    copy_gl_array_texcoord((const GLvoid*)((uintptr_t)ptr->pointer + (uintptr_t)ptr->real_pointer), ptr->type,
                           ptr->size, ptr->stride,
...
GLfloat* gl_pointer_index(vertexattrib_t* p, GLint index) {
    ...
    GL_TYPE_SWITCH(
        src, ptr, p->type,
        for (int i = 0; i < p->size; i++) { buf[i] = src[i]; }
        // zero anything not set by the pointer
        for (int i = p->size; i < 4; i++) { buf[i] = 0; },
        default : printf("LIBGL: unknown pointer type: 0x%x\n", p->type);)
    return buf;
}

GLfloat* copy_eval_double1(GLenum target, GLint ustride, GLint uorder, const GLdouble* src) {
    ...
    out = (GLfloat*)malloc(uorder * width * sizeof(GLfloat));
...
GLfloat* copy_eval_float1(GLenum target, GLint ustride, GLint uorder, const GLfloat* src) {
...
GLfloat* copy_eval_double2(GLenum target, GLint ustride, GLint uorder, GLint vstride, GLint vorder,
    ...
    if (hsize > dsize)
        out = (GLfloat*)malloc((uorder * vorder * width + hsize) * sizeof(GLfloat));
    else
        out = (GLfloat*)malloc((uorder * vorder * width + dsize) * sizeof(GLfloat));
...
GLfloat* copy_eval_float2(GLenum target, GLint ustride, GLint uorder, GLint vstride, GLint vorder, const GLfloat* src) {
    /* Additional memory is allocated to be used by the horner and
     * de Casteljau evaluation schemes.*/
    GLsizei width = get_map_width(target);
    GLfloat* out;
...
void* copy_gl_array_bgra(void* dest, const void* ptr, GLint stride, GLsizei width, GLsizei skip, GLsizei count) {
    ...
    if (!src || !(count - skip)) return NULL;

    if (!stride) stride = 4;

    if (!dest) dest = malloc(4 * sizeof(GLfloat) * (count - skip));
...
void copy_gl_pointer_color_bgra_noalloc(void* dest, const void* ptr, GLint stride, GLsizei width, GLsizei skip,
                                        GLsizei count) {
