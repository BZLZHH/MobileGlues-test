...
renderlist_t* alloc_renderlist() {
    ...
    list->matrix_val[0] = list->matrix_val[5] = list->matrix_val[10] = list->matrix_val[15] = 1.0f;
...
bool ispurerender_renderlist(renderlist_t* list) {
    ...
    if (list->calls.len) return false;
    if (list->matrix_op) return false;
    if (list->raster_op) return false;
    if (list->raster) return false;
    if (list->bitmaps) return false;
    if (list->pushattribute) return false;
    if (list->popattribute) return false;
    if (list->material || list->colormat_face || list->light || list->lightmodel || list->texgen || list->texenv)
        return false;
    if (list->fog_op) return false;
    if (list->instanceCount != 1) return false;
    if (list->pointparam_op) return false;
    if (list->mode_init == 0) return false;
    if (list->ind_lines || list->final_colors) return false;
    if (list->set_texture || list->set_tmu) return false;
...
bool islistscompatible_renderlist(renderlist_t* a, renderlist_t* b) {
    if (!globals4es.mergelist || !a) return false;
    ...
    if (a->mode_init != b->mode_init) {
        ...
        if ((a_mode == 0) || (b_mode == 0)) return false; // undetermined is not good
        if (a_mode == 4) a_mode = 3;                      // quads are handled as triangles
        if (b_mode == 4) b_mode = 3;
        if (a_mode != b_mode) return false;
    }
    if (a->use_vbo_array == 2 || b->use_vbo_array == 2) return false;
    if (a->use_vbo_indices == 2 || b->use_vbo_indices == 2) return false;
    if (!a->open || !b->open) return false;
    if (a->use_glstate) return false; // TODO: Handle this case
    /*    if ((a->indices==NULL) != (b->indices==NULL))
            return false;*/
    if (a->polygon_mode != b->polygon_mode) return false;
    if ((a->vert == NULL) != (b->vert == NULL)) return false;
    if ((a->normal == NULL) != (b->normal == NULL)) return false;
    if ((a->color == NULL) != (b->color == NULL)) return false;
    if ((a->secondary == NULL) != (b->secondary == NULL)) return false;
    if ((a->fogcoord == NULL) != (b->fogcoord == NULL)) return false;
    // check the textures
    if (a->maxtex != b->maxtex) return false;
    for (int i = 0; i < a->maxtex; i++)
        if ((a->tex[i] == NULL) != (b->tex[i] == NULL)) return false;
    if ((a->set_texture == b->set_texture) && ((a->texture != b->texture) || (a->target_texture != b->target_texture)))
        return false;
    if (!a->set_texture && b->set_texture) return false;
    // post color is only important if b as no color pointer...
    if (a->post_color && b->color == NULL) return false;
    // same for post_normal
    if (a->post_normal && b->normal == NULL) return false;
    // check instanceCount (maybe it would be better to just check if both are == 1)
    if (a->instanceCount != b->instanceCount) return false;

    // Check the size of a list, if it"s too big, don't merge...
    if ((a->len + b->len) > 60000) return false;
    if ((a->ilen + b->ilen) > 60000) return false;
...
void list_add_modeinit(renderlist_t* list, GLenum mode) {
    ...
    list->mode_inits[list->mode_init_len++].ilen =
        list->indices ? list->ilen : (list->cur_istart ? list->cur_istart : list->len);
}
...
#define alloc_a_indices newind = (GLushort*)malloc(capindices * sizeof(GLushort))
...
#define PROCESS(W, T, C)                                                                                               \
    if (list->W) {                                                                                                     \
        list->W = kh_init(W);                                                                                          \
        T *m, *m2;                                                                                                     \
        khint_t k;                                                                                                     \
        int ret;                                                                                                       \
        kh_foreach_value(a->W, m, k = kh_put(W, list->W, C, &ret); m2 = kh_value(list->W, k) = malloc(sizeof(T));      \
...
void free_renderlist(renderlist_t* list) {
    ...
    renderlist_t* next;
    do {
        if (list->mode_inits) free(list->mode_inits);
        ...
        if (list->material) {
            rendermaterial_t* m;
            kh_foreach_value(list->material, m, free(m);) kh_destroy(material, list->material);
        }
        if (list->light) {
            renderlight_t* m;
            kh_foreach_value(list->light, m, free(m);) kh_destroy(light, list->light);
        }
        if (list->texgen) {
            rendertexgen_t* m;
            kh_foreach_value(list->texgen, m, free(m);) kh_destroy(texgen, list->texgen);
        }
        if (list->texenv) {
            rendertexenv_t* m;
            kh_foreach_value(list->texenv, m, free(m);) kh_destroy(texenv, list->texenv);
        }
        if (list->lightmodel) free(list->lightmodel);

        if (list->raster && !((*list->raster->shared)--)) {
            if (list->raster->texture) fpe4es_glDeleteTextures(1, &list->raster->texture);
        ...
        if (list->bitmaps && !((*list->bitmaps->shared)--)) {
            for (int i = 0; i < list->bitmaps->count; i++)
                if (list->bitmaps->list[i].bitmap) free(list->bitmaps->list[i].bitmap);
            free(list->bitmaps->list);
            free(list->bitmaps->shared);
            free(list->bitmaps);
        }

        if (list->ind_lines) free(list->ind_lines);
        if (list->final_colors) free(list->final_colors);
        if (list->vbo_array) deleteSingleBuffer(list->vbo_array);
        if (list->vbo_indices) deleteSingleBuffer(list->vbo_indices);
...
void resize_merger_indices(int cap) {
    if (cap < glstate->merger_indice_cap) return;
    glstate->merger_indice_cap = ((glstate->merger_indice_cap + cap + 512) >> 9) << 9;
    glstate->merger_indices =
        (GLushort*)realloc(glstate->merger_indices, glstate->merger_indice_cap * sizeof(GLushort));
}

void resize_indices_renderlist(renderlist_t* list, int n) {
    if (!list->indices || list->shared_indices) return;
    if (list->use_glstate) {
        resize_merger_indices(list->ilen + n);
        list->indices = glstate->merger_indices;
    } else {
        if (list->ilen + n < list->indice_cap) return;
...
void adjust_renderlist(renderlist_t* list) {
    ...
    if (hardext.esversion == 1) // no more adjustment needed for ES2
        for (int a = 0; a < list->maxtex; a++) {
            ...
            if (list->set_texture && (list->tmu == a)) bound = fpe4es_getTexture(list->target_texture, list->texture);
...
#define pre_expand                                                                                                     \
    if (list->use_glstate) {                                                                                           \
        resize_merger_indices(renderlist_getindicesize(list));                                                         \
        indices = glstate->merger_indices;                                                                             \
    } else {                                                                                                           \
