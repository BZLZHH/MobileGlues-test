...
static void gl_changetex(int n) {
    if (glstate->bound_changed < n + 1) glstate->bound_changed = n + 1;
}
...
static void fpe_changetex(int n) {
    if (glstate->fpe_bound_changed < n + 1) glstate->fpe_bound_changed = n + 1;
}
...
generate_changetexgen(s) generate_changetexgen(t) generate_changetexgen(r) generate_changetexgen(q)
...
#define proxy_GO(constant, name)                                                                                       \
    case constant:                                                                                                     \
        if (glstate->enable.name != enable) {                                                                          \
            FLUSH_BEGINEND;                                                                                            \
            glstate->enable.name = enable;                                                                             \
            next(cap);                                                                                                 \
        }                                                                                                              \
        break
...
#define GO(constant, name)                                                                                             \
    case constant:                                                                                                     \
        if (glstate->list.pending && glstate->enable.name != enable) fpe4es_flush();                                   \
        glstate->enable.name = enable;                                                                                 \
        break;
...
#define proxy_clientGO(constant, name)                                                                                 \
    case constant:                                                                                                     \
        if (glstate->vao->name != enable) {                                                                            \
            glstate->vao->name = enable;                                                                               \
            next(cap);                                                                                                 \
        }                                                                                                              \
        break
...
#define clientGO_proxyFPE(constant, name)                                                                              \
    ...
    if (globals4es.alphahack && (cap == GL_ALPHA_TEST) && enable) {
        if (!glstate->texture.bound[glstate->texture.active][ENABLED_TEX2D]->alpha) enable = false;
    }
    noerrorShim();
#ifdef TEXSTREAM
    ...
    case GL_BLEND:
        if (glstate->enable.blend != enable) {
            FLUSH_BEGINEND;
        ...
        // cannot use clientGO_proxyFPE here, has the ClientArray are really enabled / disabled elsewhere in fact
        // (inside glDraw or list_draw)
    ...
    case GL_TEXTURE_1D:
        ...
        if (enable)
            ...
            glstate->enable.texture[glstate->texture.active] &= ~(1 << ENABLED_TEX1D);
        gl_changetex(glstate->texture.active);
        if (glstate->fpe_state) fpe_changetex(glstate->texture.active);
        break;
    case GL_TEXTURE_2D:
        if (glstate->list.pending &&
            ((glstate->enable.texture[glstate->texture.active] >> ENABLED_TEX2D) & 1) != enable)
            fpe4es_flush();
        if (enable == ((glstate->enable.texture[glstate->texture.active] >> ENABLED_TEX2D) & 1)) return; // no change
        if (enable)
            glstate->enable.texture[glstate->texture.active] |= (1 << ENABLED_TEX2D);
        else
            glstate->enable.texture[glstate->texture.active] &= ~(1 << ENABLED_TEX2D);
...
void APIENTRY_FPE4ES fpe4es_glEnable(GLenum cap) {
    ...
    if (globals4es.texstream && (cap == GL_TEXTURE_2D)) {
        if (glstate->texture.bound[glstate->texture.active][ENABLED_TEX2D]->streamed) cap = GL_TEXTURE_STREAM_IMG;
    }
...
#define isenabled(what, where)                                                                                         \
    case what:                                                                                                         \
        return glstate->enable.where
#define clientisenabled(what, where)                                                                                   \
    case what:                                                                                                         \
        return glstate->vao->where
...
GLboolean APIENTRY_FPE4ES fpe4es_glIsEnabled(GLenum cap) {
    DBG(printf("glIsEnabed(%s), list.begin=%d, list.compiling=%d, list.pending=%d\n", PrintEnum(cap),
               glstate->list.begin, glstate->list.compiling, glstate->list.pending);)
    ...
    if (glstate->list.active && !glstate->list.pending) fpe4es_flush();
