...
void APIENTRY_FPE4ES fpe4es_glRasterPos3f(GLfloat x, GLfloat y, GLfloat z) {
    ...
    fpe4es_glGetFloatv(GL_PROJECTION_MATRIX, glmatrix);
    matrix_transpose(glmatrix, projection);
    fpe4es_glGetFloatv(GL_MODELVIEW_MATRIX, glmatrix);
...
void APIENTRY_FPE4ES fpe4es_glViewport(GLint x, GLint y, GLsizei width, GLsizei height) {
    if (glstate->fbo.current_fb->id != 0) {
        gltexture_t* tex = fpe4es_getTexture(glstate->fbo.current_fb->t_color[0], glstate->fbo.current_fb->color[0]);
    ...
    if (!glstate->list.pending) PUSH_IF_COMPILING(glViewport);
    if (glstate->raster.viewport.x != x || glstate->raster.viewport.y != y || glstate->raster.viewport.width != width ||
...
void APIENTRY_FPE4ES fpe4es_glScissor(GLint x, GLint y, GLsizei width, GLsizei height) {
    if (glstate->fbo.current_fb->id != 0) {
        gltexture_t* tex = fpe4es_getTexture(glstate->fbo.current_fb->t_color[0], glstate->fbo.current_fb->color[0]);
        if (tex->fbtex_ratio > 0.0f) {
            width *= tex->fbtex_ratio;
    ...
    if (!glstate->list.pending) PUSH_IF_COMPILING(glScissor);
#ifdef AMIGAOS4
    if (x < 0) {
        width += x;
        x = 0;
    ...
    if (x + width > glstate->raster.viewport.width + glstate->raster.viewport.x)
        width = glstate->raster.viewport.width + glstate->raster.viewport.x - x;
    if (y + height > glstate->raster.viewport.height + glstate->raster.viewport.y)
        height = glstate->raster.viewport.height + glstate->raster.viewport.y - y;
#endif
    if (glstate->raster.scissor.x != x || glstate->raster.scissor.y != y || glstate->raster.scissor.width != width ||
...
void popViewport() {
    LOAD_GLES(glViewport);
    gles_glViewport(glstate->raster.viewport.x, glstate->raster.viewport.y, glstate->raster.viewport.width,
...
void init_raster(int width, int height) {
    ...
    if (!glstate->raster.data) {
        ...
        glstate->raster.raster_nwidth = w;
        glstate->raster.raster_nheight = h;
    }
    glstate->raster.raster_x1 = 0;
    glstate->raster.raster_y1 = 0;
    glstate->raster.raster_x2 = width;
    glstate->raster.raster_y2 = height;
    glstate->raster.raster_width = width;
    glstate->raster.raster_height = height;
}
...
int raster_need_transform() {
    for (int i = 0; i < 4; i++)
        if (glstate->raster.raster_scale[i] != 1.0f) return 1;
    for (int i = 0; i < 4; i++)
        if (glstate->raster.raster_bias[i] != 0.0f) return 1;
    return 0;
}
...
void raster_to_texture(rasterlist_t* r) {
    ...
    fpe4es_glPushAttrib(GL_TEXTURE_BIT | GL_ENABLE_BIT);
    GLuint old_tex_unit, old_tex;
    old_tex_unit = glstate->texture.active;
    if (old_tex_unit != 0) fpe4es_glActiveTexture(GL_TEXTURE0);
    old_tex = glstate->texture.bound[0][ENABLED_TEX2D]->glname;

    fpe4es_glPixelStorei(GL_PACK_ALIGNMENT, 1);
    fpe4es_glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    ...
    fpe4es_glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);

    if (r->texture == 0) {
        fpe4es_glEnable(GL_TEXTURE_2D);
        fpe4es_glGenTextures(1, &r->texture);
        fpe4es_glBindTexture(GL_TEXTURE_2D, r->texture);

        fpe4es_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        fpe4es_glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        fpe4es_glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        fpe4es_glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        fpe4es_glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_BASE_LEVEL,
                               0);                                      // this is to be sure texture is not npot'ed ...
        fpe4es_glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, 0); // ... if not needed
        fpe4es_glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, glstate->raster.raster_nwidth, glstate->raster.raster_nheight, 0,
                            GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    } else {
        fpe4es_glBindTexture(GL_TEXTURE_2D, r->texture);
    }
    fpe4es_glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, glstate->raster.raster_width, glstate->raster.raster_height, GL_RGBA,
                           GL_UNSIGNED_BYTE, glstate->raster.data);
    ...
    fpe4es_glBindTexture(GL_TEXTURE_2D, old_tex);
    if (old_tex_unit != 0) fpe4es_glActiveTexture(old_tex_unit + GL_TEXTURE0);
    fpe4es_glPopAttrib();
...
void bitmap_flush() {
    if (!glstate->raster.bm_drawing) return;
    // draw actual bitmap
    int old_tex_unit = glstate->texture.active;
    if (old_tex_unit) fpe4es_glActiveTexture(GL_TEXTURE0);

    GLuint old_tex = glstate->texture.bound[0][ENABLED_TEX2D]->glname;
    GLuint old_active = glstate->enable.texture[0];

    if (IS_TEX1D(old_active)) fpe4es_glDisable(GL_TEXTURE_1D);
    if (!IS_TEX2D(old_active)) fpe4es_glEnable(GL_TEXTURE_2D);
    if (IS_TEX3D(old_active)) fpe4es_glDisable(GL_TEXTURE_3D);
    if (IS_TEXTURE_RECTANGLE(old_active)) fpe4es_glDisable(GL_TEXTURE_RECTANGLE_ARB);
    if (IS_CUBE_MAP(old_active)) fpe4es_glDisable(GL_TEXTURE_CUBE_MAP);

    if (!glstate->raster.bm_texture) {
        fpe4es_glGenTextures(1, &glstate->raster.bm_texture);
        fpe4es_glBindTexture(GL_TEXTURE_2D, glstate->raster.bm_texture);
    ...
    if (glstate->raster.bm_tnwidth < glstate->raster.bm_width ||
        glstate->raster.bm_tnheight < glstate->raster.bm_height) {
        ...
        fpe4es_glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, glstate->raster.bm_tnwidth, glstate->raster.bm_tnheight, 0,
                            GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    }
    ...
    if (sx == 0 && sy == 0 && ex == glstate->raster.bm_width && ey == glstate->raster.bm_height) {
        fpe4es_glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, glstate->raster.bm_width, glstate->raster.bm_height, GL_RGBA,
                               GL_UNSIGNED_BYTE, glstate->raster.bitmap);
    } else {
        int alloc = 4 * ex * ey;
        fpe4es_scratch(alloc);
        for (int i = 0; i < ey; i++)
            memcpy((char*)glstate->scratch + 4 * i * ex,
                   (char*)glstate->raster.bitmap + 4 * (sx + (sy + i) * glstate->raster.bm_width), ex * 4);
        fpe4es_glTexSubImage2D(GL_TEXTURE_2D, 0, sx, sy, ex, ey, GL_RGBA, GL_UNSIGNED_BYTE, glstate->scratch);
    }

    fpe4es_blitTexture(glstate->raster.bm_texture, sx, sy, ex, ey, glstate->raster.bm_tnwidth,
                       glstate->raster.bm_tnheight, 1.f, 1.f, 0, 0, // vp is default here
                       sx, sy, BLIT_ALPHA);

    glstate->raster.bm_drawing = 0;

    if (IS_TEX1D(old_active)) fpe4es_glEnable(GL_TEXTURE_1D);
    if (!IS_TEX2D(old_active)) fpe4es_glDisable(GL_TEXTURE_2D);
    if (IS_TEX3D(old_active)) fpe4es_glEnable(GL_TEXTURE_3D);
    if (IS_TEXTURE_RECTANGLE(old_active)) fpe4es_glEnable(GL_TEXTURE_RECTANGLE_ARB);
    if (IS_CUBE_MAP(old_active)) fpe4es_glEnable(GL_TEXTURE_CUBE_MAP);

    if (old_tex != glstate->raster.bm_texture) fpe4es_glBindTexture(GL_TEXTURE_2D, old_tex);

    if (old_tex_unit) fpe4es_glActiveTexture(GL_TEXTURE0 + old_tex_unit);
}
...
void APIENTRY_FPE4ES fpe4es_glBitmap(GLsizei width, GLsizei height, GLfloat xorig, GLfloat yorig, GLfloat xmove,
    ...
    /*printf("glBitmap, xy={%f, %f}, xyorig={%f, %f}, size={%u, %u}, zoom={%f, %f}, viewport={%i, %i, %i, %i}\n",
        glstate->raster.rPos.x, glstate->raster.rPos.y, xorig, yorig, width, height, glstate->raster.raster_zoomx,
       glstate->raster.raster_zoomy, glstate->raster.viewport.x, glstate->raster.viewport.y,
    ...
    if (glstate->list.active) {
        ...
        if (list->bitmaps->count == list->bitmaps->cap) {
            list->bitmaps->cap += 8;
            list->bitmaps->list =
                (bitmap_list_t*)realloc(list->bitmaps->list, list->bitmaps->cap * sizeof(bitmap_list_t));
        }
    ...
    if (((!width && !height) || (bitmap == 0)) && glstate->raster.rPos.x + xmove >= 0 &&
        glstate->raster.rPos.y + ymove >= 0) {
    ...
    if (glstate->raster.bm_alloc < glstate->raster.viewport.width * glstate->raster.viewport.height * 4) {
        if (glstate->raster.bitmap) free(glstate->raster.bitmap);
...
void APIENTRY_FPE4ES fpe4es_glDrawPixels(GLsizei width, GLsizei height, GLenum format, GLenum type, const GLvoid* data) {
    ...
    if (type == GL_BITMAP) {
        fpe4es_glBitmap(width, height, 0, 0, 0, 0, data);
        return;
    }
    ...
    /*printf("glDrawPixels, xy={%f, %f}, size={%i, %i}, format=%s, type=%s, zoom={%f, %f}, viewport={%i, %i, %i, %i}\n",
       ...
       glstate->raster.viewport.y, glstate->raster.viewport.width, glstate->raster.viewport.height);*/
    ...
    if (!pixel_convert(data, &dst, bmp_width, height, format, type, GL_RGBA, GL_UNSIGNED_BYTE, 0,
                       1)) { // pack_align is forced to 1 when drawing
        return;
    }
    ...
    if (pixtrans) {
        for (int y = 0; y < height; y++) {
            ...
            from = pixels +
                   4 * (glstate->texture.unpack_skip_pixels + (y + glstate->texture.unpack_skip_rows) * bmp_width);
    ...
    if (glstate->list.active) {
        ...
        *r->shared = 0;
        ...
        if (r->texture && (width > r->nwidth || height > r->nheight)) {
            fpe4es_glDeleteTextures(1, &r->texture);
            r->texture = 0;
        }
    }
    raster_to_texture(r);
...
void render_raster_list(rasterlist_t* rast) {
    // printf("render_raster_list, rast->x/y=%f/%f rast->width/height=%i/%i, rPos.x/y/z=%f/%f/%f, rast->zoomxy=%f/%f
    // raster->texture=%u\n", rast->xorig, rast->yorig, rast->width, rast->height, glstate->raster.rPos.x,
    // glstate->raster.rPos.y, glstate->raster.rPos.z, rast->zoomx, rast->zoomy, rast->texture);
    if (rast->texture)
        fpe4es_blitTexture(rast->texture, 0.f, 0.f, rast->width, rast->height, rast->nwidth, rast->nheight, rast->zoomx,
                           rast->zoomy, 0, 0, // vp is default here
...
void APIENTRY_FPE4ES fpe4es_glPixelMapfv(GLenum map, GLsizei mapsize, const GLfloat* values) {
    ...
    int need_pot = 0;
    if (map == GL_PIXEL_MAP_I_TO_I || map == GL_PIXEL_MAP_S_TO_S || map == GL_PIXEL_MAP_I_TO_R ||
        map == GL_PIXEL_MAP_I_TO_G || map == GL_PIXEL_MAP_I_TO_B || map == GL_PIXEL_MAP_I_TO_A)
        need_pot = 1;
    ...
    void* array = NULL;
    int* size = NULL;
    if (!map_pixelmap(map, &wf, &size, &array)) return;
    noerrorShim();
    if (wf) {
