...
int tex_setup_needchange(GLuint itarget) {
    ...
    if (itarget == ENABLED_TEXTURE_RECTANGLE) return 1;
    return 0;
}
...
void APIENTRY_FPE4ES fpe4es_glBindTexture(GLenum target, GLuint texture) {
    ...
    if ((target != GL_PROXY_TEXTURE_2D) && glstate->list.compiling && glstate->list.active && !glstate->list.pending) {
        ...
        tex = fpe4es_getTexture(target, texture);
...
void APIENTRY_FPE4ES fpe4es_glTexParameterfv(GLenum target, GLenum pname, const GLfloat* params) {
    ...
    if (!samplerParameterfv(&texture->sampler, pname, params)) {
        ...
        case GL_TEXTURE_MAX_LEVEL:
            if (texture) texture->max_level = param;
            return; // not on GLES
        ...
        case GL_GENERATE_MIPMAP:
            ...
            if (hardext.esversion > 1) {
                /*if(texture->valid) {
                    ...
                    fpe4es_glGenerateMipmap(rtarget);
                }*/
                return;
            }
...
void APIENTRY_FPE4ES fpe4es_glTexParameterf(GLenum target, GLenum pname, GLfloat param) {
    fpe4es_glTexParameterfv(target, pname, &param);
}
void APIENTRY_FPE4ES fpe4es_glTexParameteri(GLenum target, GLenum pname, GLint param) {
    GLfloat fparam = param;
    fpe4es_glTexParameterfv(target, pname, &fparam);
}

void APIENTRY_FPE4ES fpe4es_glTexParameteriv(GLenum target, GLenum pname, const GLint* params) {
    GLfloat fparams[4];
    fparams[0] = *params;
    if (pname == GL_TEXTURE_BORDER_COLOR)
        for (int i = 1; i < 4; ++i)
            fparams[i] = params[i];
    fpe4es_glTexParameterfv(target, pname, fparams);
}
    ...
    if (list) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (k != kh_end(list)) {
                ...
                if (tex->binded_fbo) {
                    if (tex->renderdepth) fpe4es_glDeleteRenderbuffers(1, &tex->renderdepth);
                    if (tex->renderstencil) fpe4es_glDeleteRenderbuffers(1, &tex->renderstencil);
                }
                errorGL();
#ifdef TEXSTREAM
...
void APIENTRY_FPE4ES fpe4es_glPixelStorei(GLenum pname, GLint param) {
...
void realize_textures(int drawing) {
    ...
    void gles_glTexParameteri(glTexParameteri_ARG_EXPAND); // LOAD_GLES(glTexParameteri);
#ifdef TEXSTREAM
    DBG(printf("realize_textures(%d), glstate->bound_changed=%d, glstate->enable.texture[0]=%X "
