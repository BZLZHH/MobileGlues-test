...
void fpe4es_glTexImage2D(GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border,
                         GLenum format, GLenum type, const GLvoid* data) {
    DBG(printf("glTexImage2D on target=%s with unpack_row_length(%i), size(%i,%i) and skip(%i,%i), "
    ...
    if (bound->binded_fbo &&
        ...
        if (data != NULL) printf("LIBGL: Warning, Depth/stencil texture resized and with data\n");
        // get new size...
        ...
        if (bound->binded_attachment == GL_DEPTH_ATTACHMENT ||
            ...
            if (bound->renderdepth) {
                fpe4es_glBindRenderbuffer(GL_RENDERBUFFER, bound->renderdepth);
                fpe4es_glRenderbufferStorage(GL_RENDERBUFFER,
        ...
        if ((bound->binded_attachment == GL_STENCIL_ATTACHMENT ||
             bound->binded_attachment == GL_DEPTH_STENCIL_ATTACHMENT) &&
            bound->renderstencil) {
            fpe4es_glBindRenderbuffer(GL_RENDERBUFFER, bound->renderstencil);
            fpe4es_glRenderbufferStorage(GL_RENDERBUFFER, GL_STENCIL_INDEX8, nwidth, nheight);
            fpe4es_glBindRenderbuffer(GL_RENDERBUFFER, 0);
        }
        // all done, exit
        errorGL();
        ...
        if (!(globals4es.texstream && bound->streamed)) {
            if ((target != GL_TEXTURE_RECTANGLE_ARB) && (globals4es.automipmap != 3) &&
                ...
                if ((itarget != ENABLED_CUBE_MAP && target != GL_TEXTURE_RECTANGLE_ARB) &&
                    ...
                    bound->mipmap_need = 0;
                    fpe4es_glTexParameteri(rtarget, GL_TEXTURE_MIN_FILTER,
            ...
            if (height != nheight || width != nwidth) {
                ...
                errorGL();
                DBG(printf("gles_glTexImage2D(%d, %d, %s, %d, %d, %d, %s, %s, 0x%x)\n", rtarget, level,
                           PrintEnum(internalformat), width, height, border, PrintEnum(format), PrintEnum(type),
                           pixels);)
            ...
            if (pixels)
                fpe4es_glTexSubImage2D(rtarget, level, 0, 0, width, height, format, type,
...
void fpe4es_glTexSubImage2D(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height,
                            GLenum format, GLenum type, const GLvoid* data) {
    ...
    if (globals4es.texstream && bound->streamed) {
        ...
        errorGL();
        DBG(printf("gles_glTexSubImage2D(%d, %d, %d, %d, %d, %d, %s, %s, 0x%x)\n", rtarget, level, xoffset, yoffset,
                   width, height, PrintEnum(format), PrintEnum(type), pixels);)
...
// 1d stubs
void fpe4es_glTexImage1D(GLenum target, GLint level, GLint internalFormat, GLsizei width, GLint border, GLenum format,
                         GLenum type, const GLvoid* data) {

    // TODO: maybe too naive to force GL_TEXTURE_2D here?
    fpe4es_glTexImage2D(GL_TEXTURE_1D, level, internalFormat, width, 1, border, format, type, data);
}
void fpe4es_glTexSubImage1D(GLenum target, GLint level, GLint xoffset, GLsizei width, GLenum format, GLenum type,
                            const GLvoid* data) {

    fpe4es_glTexSubImage2D(GL_TEXTURE_1D, level, xoffset, 0, width, 1, format, type, data);
}

GLboolean fpe4es_glIsTexture(GLuint texture) {
...
void fpe4es_glTexStorage1D(GLenum target, GLsizei levels, GLenum internalformat, GLsizei width) {
    DBG(SHUT_LOGD("glTexStorage1D(%s, %d, %s, %d)\n", PrintEnum(target), levels, PrintEnum(internalformat), width);)
    fpe4es_glTexImage1D(target, 0, internalformat, width, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
}
void fpe4es_glTexStorage2D(GLenum target, GLsizei levels, GLenum internalformat, GLsizei width, GLsizei height) {
    ...
    if ((internalformat == GL_COMPRESSED_RGB_S3TC_DXT1_EXT || internalformat == GL_COMPRESSED_SRGB_S3TC_DXT1_EXT) &&
        ...
        fpe4es_glTexImage2D(target, 0, internalformat, width, height, 0, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, NULL);
    else if (((internalformat == GL_COMPRESSED_RGBA_S3TC_DXT1_EXT ||
               internalformat == GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT1_EXT)) &&
             !globals4es.avoid16bits)
        fpe4es_glTexImage2D(target, 0, internalformat, width, height, 0, GL_RGBA, GL_UNSIGNED_SHORT_5_5_5_1, NULL);
    else if ((internalformat == GL_COMPRESSED_RGBA_S3TC_DXT3_EXT ||
              internalformat == GL_COMPRESSED_RGBA_S3TC_DXT5_EXT ||
              internalformat == GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT3_EXT ||
              internalformat == GL_COMPRESSED_SRGB_ALPHA_S3TC_DXT5_EXT) &&
             !globals4es.avoid16bits)
        fpe4es_glTexImage2D(target, 0, internalformat, width, height, 0, GL_RGBA, GL_UNSIGNED_SHORT_4_4_4_4, NULL);
    else
        fpe4es_glTexImage2D(target, 0, internalformat, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
    ...
    if (levels > 1 && isDXTc(internalformat)) {
        ...
        for (int i = 1; i <= mlevel; ++i)
            fpe4es_glTexImage2D(target, i, internalformat, nlevel(width, i), nlevel(height, i), 0, bound->format,
                                bound->type, NULL);
        noerrorShim();
        return;
    }
